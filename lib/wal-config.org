...
(defvar wal-ascii-whale--ignored-buffers '("COMMIT_EDITMSG")
...
(defun wal-ascii-whale--maybe-display (buffer &optional no-record &rest _args)
  ...
  (when wal-config-show-whale-animation
    (and-let* (((not no-record))
               ...
               ((not (and-let* ((name (buffer-file-name buffer)))
                       (seq-find (lambda (it) (string-match-p it name)) wal-ascii-whale--ignored-buffers))))
               ((null (buffer-local-value 'wal-ascii-whale-parent-buffer buffer)))
...
(defun wal-ascii-whale--on-find-file ()
  "Maybe display animation for new file."
  (wal-ascii-whale--maybe-display (current-buffer)))

(wal-on-boot config
  (advice-add 'switch-to-buffer :after #'wal-ascii-whale--maybe-display)
  (add-hook 'find-file-hook #'wal-ascii-whale--on-find-file))
#+END_SRC
