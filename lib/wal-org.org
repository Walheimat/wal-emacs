...
(defun wal-agenda--non-agenda-buffer-p (buffer)
  "Verify that BUFFER does not contribute to the agenda."
  (not (wal-agenda-buffer-p buffer)))
...
(defun wal-org-capture--find-project-tasks-heading ()
  ...
  (if-let* ((project (project-current t))
            ...
            (marker (or (and-let* ((file wal-org-capture-tasks-file)
