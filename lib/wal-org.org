...
(defun wal-org-refile (&optional arg)
  "Prefix-argument-based `org-refile'.

If called with numeric prefix ARG 5, this command always refiles
normally. If called with this will set the agenda files to the
`default-directory'. Otherwise this command will set the agenda files to
the roam directory for roam buffers or just pass on the argument ARG."
  (interactive "P")
