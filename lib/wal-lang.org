...
(junk-expand swift
  ...
  :packages (swift-mode lsp-sourcekit))
#+end_src
...
(use-package swift-mode
  ...
  (harpoon swift-mode
    :lsp (:format t)
    :ligatures (".." "->" "??")
    :messages ("At its core not a bad language")))
#+end_src
...
(use-package lsp-sourcekit
  ...
  :after swift-mode
  :custom
  (lsp-sourcekit-executable "~/.local/share/swiftly/bin/sourcekit-lsp"))
#+end_src
...
(use-package vue-mode
  ...
  (harpoon vue-mode
    :lsp t
    :messages ("What a beautiful view")
    :ligatures ("=>" "!==" "===" "!!")

    ;; This mode is derived from `html-mode'.
    (corfu-mode +1)))
#+end_src
...
(defun wal-maven-test-match-file ()
  "Find the file associated with the matched maven test error."
  (save-match-data
    (when-let* ((file (match-string 2))
                (file-regexp (concat (substring-no-properties file) "\\'"))
                (files (project-files (project-current)))
                (file (seq-find (lambda (it) (string-match-p file-regexp it)) files)))

      file)))

(defvar wal-maven-test-compilation-error-regexp
  '(maven-test
    "\t\\at .+(\\(.+.java\\):\\([0-9]+\\))" ; Regexp.
    wal-maven-test-match-file               ; File.
    2                                       ; Line.
    nil                                     ; Column.
    nil                                     ; Type.
    1))                                     ; Hyperlink
...
(use-package lsp-java
  ...
  ;; Add regular expression for Maven errors.
  (add-to-list 'compilation-error-regexp-alist 'maven-test)
  (add-to-list
   'compilation-error-regexp-alist-alist
   wal-maven-test-compilation-error-regexp)
