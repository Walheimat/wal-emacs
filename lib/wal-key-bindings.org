...
+ =M-o= to switch to the most recently used window (=C-M-o= switch to
  other buffer).
...
+ =H-\= captures note with =org-roam= (=H-M-\= opens custom
  =transient=)
+ =H-h= finds file in =project= (=H-M-h= finds file in current
  directory)
...
+ =H-l= jumps to line with =avy= (beginning of line if called with
  =C-u=, =H-M-l= goes to line using =consult=)
...
(with-no-warnings
  (with-eval-after-load 'general
    ...
    (eval-after-load 'which-key
      (which-key-add-key-based-replacements (wal-prefix-user-key "M-k") "completionist"))
