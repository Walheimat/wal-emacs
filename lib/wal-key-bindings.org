...
+ =H-y= switches projects (=H-M-y= switches to parent project)
+ =H-\= finds a note with =org-roam= (=H-M-\= opens custom =transient=)
+ =H-p= finds file in =project= (=H-M-p= finds file in current directory)
...
+ =H-h= jumps to register, =H-M-h= stores point in register
...
+ =H-'= goes to mark with =consult= (=H-M-'= goes to global mark)
