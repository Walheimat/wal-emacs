...
+ =C-c r= to =recompile=
+ =C-c s= for =flyspell=
+ =C-c t= to clock out and =C-c T= to take notes for current
  =org-clock= task (go-to with =C-u 0=)
...
+ =H-p= stores point in register, =H-y= stores window configuration to
  register and =H-h= jumps to register
+ =H-<SPC>= completes at point for modes configured by =harpoon=
...
(with-no-warnings
  ...
  (global-set-key (kbd (wal-prefix-user-key "<SPC>")) #'completion-at-point)
