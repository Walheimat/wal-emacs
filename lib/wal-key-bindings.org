...
Most bindings are quick-access actions, some have an alternate action bound in prefixed =a=, some bind a command map or transient in prefixed =c=.
...
(use-package general
  ...
  (general-define-key :prefix (wal-prefix-user-key "c") :prefix-map 'wal-command-map)

  (general-define-key :prefix (wal-prefix-user-key "a") :prefix-map 'wal-alternate-map)
