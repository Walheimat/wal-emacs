...
+ =H-M-]= calls =dap-debug= (=H-]= binds custom command map)
+ =H-M-[= calls =wal-lsp-dwim= in LSP buffers (=H-[= binds =lsp-command-map=)
...
+ =H-M-,= to rerun the last =ship-mate= command (=H-,= binds =ship-mate-command-map=)
