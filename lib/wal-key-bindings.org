...
+ =H-p= switches projects (=H-M-p= switches to parent project)
+ =H-\= finds a note with =org-roam= (=H-M-\= opens custom =transient=)
+ =H-y= finds file in =project= (=H-M-y= finds file in current directory)
+ =H-k= calls =consult= to find a place using =imenu= or =outline=
+ =H-h= calls =consult= to search for a term (=H-M-h= does using the symbol at point as the candidate)
+ =H-j=
