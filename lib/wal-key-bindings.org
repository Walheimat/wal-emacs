...
| =H-j=                    |                                         | =wal-avy-goto-word=               | =avy-goto-char-in-line=                | -             |
...
(defcustom wal-hyper-prefix "H"
  "The key to consider the hyper prefix."
  :type 'key-sequence
  :group 'wal-key-bindings)
...
(defsubst wal-prefix-user-key (user-key)
  "Prefix USER-KEY."
  (let ((prefix (concat wal-hyper-prefix "-")))
...
(with-no-warnings
  ...
  (define-key function-key-map (kbd "<f11>") #'event-apply-hyper-modifier)
