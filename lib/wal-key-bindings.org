...
Function key =<f5>= is bound to =whaler=, a =transient= that binds
various commands to interact with this configuration (updating,
tangling).
...
(defcustom wal-leaders '(("6" . adjunct)
                         ("7" . editor)
                         ("8" . major)
                         ("9" . ambassador)
                         ("0" . seeker))
...
#+BEGIN_SRC emacs-lisp
(defvar wal-general-leaders '(editor ambassador adjunct seeker)
...
(with-no-warnings
  (with-eval-after-load 'general
    ...
    (general-create-definer administrator :prefix "<f6>")
    (that-key "administrator" :key "<f6>")
