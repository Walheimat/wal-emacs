...
+ =H-p= captures a note with =org-roam= (=H-M-p= opens custom =transient=)
...
+ =H-<SPC>= clocks in (or out if called with prefix; prefix =0= clocks in without continuation; =H-M-<SPC>= takes a note for the currently clocked item))
...
+ =H-/= calls =cape-dabbrev= (=H-M-/= is the prefix for leader key =completionist=)
+ =H-\= calls =wal-supernova= (=H-M-\= calls =wal-consult-display-buffer= to display a buffer without selecting it).
...
(with-no-warnings
  (with-eval-after-load 'general
    ...
    (general-create-definer completionist :prefix (wal-prefix-user-key "M-/"))
