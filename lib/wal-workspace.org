...
`comint-mode' instead."
  ...
  (let ((function-name (intern (format "wal-project-%s" name)))
    ...
    `(progn
       (defvar-local ,variable-name ,default ,(format "Default for `%s'." function-name))

       (defun ,function-name (&optional arg)
         ,(concat (capitalize (symbol-name name))
                  " the current project.\n"
                  "This will use "
                  (if comint "`comint-mode'" "`compile-mode'")
                  " unless ARG is t, then it will use "
                  (if comint "`compile-mode'" "`comint-mode'")
                  ".")
         (interactive "P")

         ,(if comint
              `(wal-project-command ',name (null arg))
            `(wal-project-command ',name (not (null arg)))))
       ...
       (put ',variable-name 'safe-local-variable #'wal-project-command--valid-default-p))))
...
(use-package project
  ...
  (wal-project-create-command install :comint t)
  (wal-project-create-command lint)
  (wal-project-create-command publish :comint t)
  (wal-project-create-command clean :default "make clean")
  (wal-project-create-command coverage :key "o" :default "make coverage")
  (wal-project-create-command run :comint t)
  (wal-project-create-command update)
