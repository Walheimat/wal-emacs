...
`comint-mode' instead."
  ...
  (let ((function-name (intern (format "wal-project-%s" name)))
        ...
        (reverse-var (intern (format "wal-project-%s-reverse-mode" name)))
        (key (or key (substring (symbol-name name) 0 1))))

    `(progn
       (defvar-local ,default-var ,default ,(format "Default for `%s'." function-name))
       (defvar-local ,reverse-var nil "Whether to reverse `comint' usage.")
       ...
       (defun ,function-name (&optional arg)
         ,(concat (capitalize (symbol-name name))
                  ...
                  " unless ARG or `" (symbol-name reverse-var)
                  "' is t, then it will use "
                  (if comint "`compile-mode'" "`comint-mode'")
                  ".")
         (interactive "P")

         ,(if comint
              `(wal-project-command ',name (and (null arg) (not ,reverse-var)))
            `(wal-project-command ',name (or (not (null arg)) ,reverse-var))))
       ...
       (put ',reverse-var 'safe-local-variable #'booleanp))))
...
(use-package project
  ...
  (wal-project-create-command install :comint t :default "make install")
