...
This is a mail client for (and part of) =mu= (install =maildir-utils= or build from [[https://github.com/djcb/mu][source]]). It uses =offlineimap= that needs to be configured for your server. It's bound in [[file:wal-key-bindings.org::*Ambassador][ambassador]].
...
I recommend putting personal data in a =site-start.el= file somewhere in your =load-path=.
