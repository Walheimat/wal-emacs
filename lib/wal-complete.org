...
#+BEGIN_SRC emacs-lisp
(defvar wal-no-corfu '(ship-mate-execute)
  "Commands that will make sure `corfu-auto' is disabled.")

(defun wal-corfu-enable-in-minibuffer ()
  "Enable Corfu in the minibuffer if `completion-at-point' is bound.

Disable `corfu-auto' if `this-command' is blacklisted."
  (when (where-is-internal #'completion-at-point (list (current-local-map)))
    (corfu-mode 1)

    (when (memq this-command wal-no-corfu)
