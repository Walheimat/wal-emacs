...
(declare-function wal-tab-buffers--has-buffers-p "wal-windows.el")
...
(defcustom wal-consult-buffer-narrow-source 'tab
  "Source to use when pre-narrowing `consult-buffer'."
  :type '(choice (const :tag "Tab buffers" tab)
...
(use-package corfu
  :hook
  (((org-mode prog-mode prog-like eshell-mode inferior-python-mode) . corfu-mode)
...
#+BEGIN_SRC emacs-lisp
(defun wal-marginalia-junk-annotate (candidate)
...
(use-package marginalia
  ...
  (add-to-list 'marginalia-annotator-registry '(expansion-pack wal-marginalia-junk-annotate builtin none))
                      ...
                      (pcase wal-consult-buffer-narrow-source
                        ('project
                         (when-let* ((root (consult--project-root))
                           ...
                           ?p))
                        ('tab
                         (when (wal-tab-buffers--has-buffers-p) ?t))))
