...
#+BEGIN_SRC emacs-lisp
(defun wal-vertico--no-cycle (func &rest args)
  "Run FUNC with ARGS while cycling is disabled."
...
(use-package vertico
  ...
  (vertico-multiform-commands '((tab-switch flat)
                                ...
                                (consult-buffer flat)
                                (wal-consult-display-buffer flat)
                                (ace-window flat)
...
(use-package marginalia
  ...
  :defines (marginalia-annotators marginalia-annotator-registry marginalia-command-categories))
#+END_SRC
...
(use-package embark
  ...
  (embark-cycle-key (wal-prefix-user-key ";"))
...
(defun wal-consult-display-buffer ()
  "Select a buffer and display (not switch to) it.

This uses `display-buffer-pop-up-window'."
  (interactive)
  ...
  (let ((display-buffer-alist nil)
        (consult--buffer-display (lambda (cand &rest _)
                                   (display-buffer cand 'display-buffer-pop-up-window))))
...
(defun wal-consult-org-agenda-take-note (&optional arg)
...
the item first."
  ...
  (declare-function org-add-note "ext:org.el")
...
(defun wal-consult-line-symbol-at-point ()
  "Pass symbol at point as first parameter."
  (interactive)
...
(defun wal-consult-narrow-for-agenda ()
  "Pre-narrow `consult-buffer' if it's run from the agenda buffer.

This also sets the `consult--buffer-display' to use
`switch-to-buffer-other-window'."
  (and-let* (((eq this-command 'consult-buffer))
             (original-mode (buffer-local-value 'major-mode minibuffer--original-buffer))
             ((eq original-mode 'org-agenda-mode)))

    (with-current-buffer minibuffer--original-buffer
      (setq-local consult--buffer-display 'switch-to-buffer-other-window))

    (setq-local unread-command-events (list ?a 32))))
#+end_src
...
(use-package consult
  ...
  :hook (minibuffer-setup . wal-consult-narrow-for-agenda)
  ...
  ;; Don't cycle when using certain commands.
  (advice-add 'consult-ripgrep :around 'wal-vertico--no-cycle)
  (advice-add 'consult-line :around 'wal-vertico--no-cycle)
  ...
  (("<SPC>" . wal-consult-clock)
   ("M-<SPC>" . wal-consult-org-agenda-take-note)
   ("k" . wal-consult-place)
   ("'" . wal-consult-project)
   ("M-l" . consult-goto-line)
   (";" . consult-line)
   ("M-;" . wal-consult-line-symbol-at-point)

   ("u" . consult-buffer)
   ("M-u" . consult-recent-file)
   ("C-u" . consult)
   ("M-\\" . wal-consult-display-buffer)))
#+END_SRC
