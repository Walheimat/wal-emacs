...
If DEDICATED is a boolean, if WIDTH is nil, default to 50%, if
HEIGHT is nil default to 10 lines, if NO-OTHER is t set the
`window-parameter'."
...
(cl-defun wal-display-buffer-same-place-or-faraway (buffer &key width height below dedicated no-other)
  "Display BUFFER where it is or far away.

If BELOW is t, display below. Otherwise tries to reuse window
first, then checks for another frame and lastly pops up a window.
    ...
    (add-to-list 'display-buffer-alist
                 `(,condition
				   ,(delq nil `(display-buffer-reuse-window
								...
								,(when below 'display-buffer-below-selected)
