...
(use-package partial-recall
  ...
  (define-key wal-command-map "i" partial-recall-command-map)
  (that-key-in-map "partial-recall" :key "i")

  :bind
  (:map wal-alternate-map
   ("M-i" . partial-recall-menu))
...
(use-package junk
  ...
  (with-eval-after-load 'marginalia
    (when (boundp 'marginalia-annotators)
      (add-to-list 'marginalia-annotators '(expansion-pack junk-annotate builtin none)))
...
The package configuration defines =ship-mate-{build,install,clean,execute,test}= commands. Each of these commands have their own per-project history that can be pre-filled through defaults and dir-local variables. The commands are provided through a prefix map bound to =C-M-s-c-,=.
...
(use-package ship-mate
  ...
  (ship-mate-create-command run :multiple t)

  (wdb-faraway "\\*ship-mate")

  (define-key wal-command-map "," ship-mate-command-map)
  (define-key wal-alternate-map "," ship-mate-quick-command-map)
  (that-key-in-map "ship-mate" :key ",")
  (that-key-in-map "ship-mate" :key "," :alternate t)

  :custom
  (ship-mate-prefix-read-command t)
