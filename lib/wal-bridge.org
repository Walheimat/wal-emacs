...
(use-package partial-recall
  ...
  (that-key "partial-recall" :user-key "C-i")
  ...
  :wal-bind-keymap
  ("C-i" . partial-recall-command-map))
#+end_src
...
(use-package ship-mate
  ...
  (that-key "ship-mate" :user-key "C-,")
  (that-key "ship-mate commands" :user-key "M-,")
