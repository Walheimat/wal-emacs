...
+ =C-+= expands region.
+ =C-c a a= runs ag. _Requires ag_!
+ =C-c a p= runs ag-project.
+ =C-c b b= toggles bookmarks.
+ =C-c b s= show bookmarks.
+ =C-c c d= duplicates the current line (or region).
+ =C-c c k= kills all other buffers.
+ =C-c c o= opens file with outside program.
+ =C-c d= opens docker.
+ =C-c e= opens eshell.
...
+ =C-c q= opens the timemachine.
+ =C-c s= uses swiper to search.
+ =C-c v c= jumps to char with avy.
+ =C-c v l= jumps to line with avy.
...
(use-package git-timemachine
  :chords ("tm" . git-timemachine-toggle)
...
Let's try out some guitar-playin'.
...
(use-package yasnippet
  ...
  :hook ((prog-mode      . yas-minor-mode)
         (yas-minor-mode . (lambda () (yas-reload-all)))))
