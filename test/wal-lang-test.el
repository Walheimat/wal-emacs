...
(ert-deftest test-wal-lsp-pyright-install-stubs ()
  (bydi-with-mock ((wal-in-python-project-p . #'ignore))

    (should-error (wal-lsp-pyright-install-stubs) :type 'user-error))

  (let ((directory-exists t))

    (bydi-with-mock ((wal-in-python-project-p . #'always)
...
(ert-deftest test-wal-prettier-refresh ()
  ...
  (let ((prettier-processes nil))
    ...
    (bydi-with-mock (prettier--quit-all-processes
                     prettier-prettify
                     run-with-timer
...
(ert-deftest test-wal-markdown-view ()
  (bydi-with-mock (mixed-pitch-mode
                   markdown-mode
                   markdown-view-mode)
...
(ert-deftest test-wal-find-dart-flutter-sdk-dir ()
  (bydi-with-mock ((executable-find . (lambda (x) (format "/usr/bin/%s" x)))
...
(ert-deftest test-wal-lsp-dart-service-uri ()
  (bydi-with-mock ((lsp-workspace-get-metadata . (lambda (_) (error "testing"))))

    (should (string-equal "Couldn’t get service URI: testing" (wal-lsp-dart-service-uri))))

  (bydi-with-mock ((lsp-workspace-get-metadata . (lambda (_) "test-uri")))
...
(ert-deftest test-wal-java-test-dwim ()
  (bydi-with-mock (dap-java-run-test-method
                   transient-set
...
(ert-deftest test-wal-junit-match-file ()
  (let ((matched nil))
    ...
    (bydi-with-mock ((buffer-list . (lambda () (list "a" "b")))
...
(ert-deftest test-wal-maybe-use-custom-css-checker ()
  ...
  (bydi-with-mock ((executable-find . #'always))
