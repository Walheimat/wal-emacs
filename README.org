...
Some command maps are also bound:
...
actions, mostly through =transient= bound to the same key (=.= by
default).

*** Captain

Leader key =captain= (=/= by default) gives access to minor-mode[fn:11]
command maps and transients. Always active:

+ =f= for =flycheck=
+ =i= for =perspective= (=I= for full command map)
+ =p= for =projectile= (=P= for full command map)
+ =w= for =winner=.

If the respective minor mode is active, it also has:
...
The sink for =captain= toggles minor modes.

*** Lieutenant

Leader key =lieutenant= (='= by default) is currently used for editor
adjustments (text and window size) using custom =hydra= maps.
...
(defvar wal/leader-with-sink '(wal/general wal/colonel wal/captain wal/lieutenant))
...
(wal/colonel "q" '(query-replace :wk "replace"))
(wal/colonel-sink "q" '(query-replace-regexp :wk "regexp replace"))
...
(use-package follow-mode
  ...
  :general
  (wal/captain-sink "o" '(follow-mode :wk "follow"))
  :ensure nil)
#+end_src
...
(use-package multiple-cursors
  ...
  :general
  (wal/colonel "." '(mc/mark-all-like-this :wk "mark all like this"))
  (wal/colonel-sink "." '(mc/edit-ends-of-lines :wk "mark region ends")))
#+END_SRC
...
(use-package yasnippet
  ...
  (:map yas-minor-mode-map
   ("y" . yas-expand))
  :general
  (wal/captain "y" '(wal/yas :wk "yasnippet"))
...
(use-package crux
  ...
  :general
  (wal/colonel "d"
    '(crux-duplicate-current-line-or-region :wk "duplicate"))
  (wal/colonel-sink "d"
  '(crux-duplicate-and-comment-current-line-or-region :wk "duplicate and comment")))
#+END_SRC
...
(use-package avy
  ...
  ;; Lines.
  (wal/colonel
    "c" '(avy-copy-line :wk "copy line")
    "x" '(avy-kill-whole-line :wk "delete line")
    "m" '(avy-move-line :wk "move line")
    "w" '(avy-kill-ring-save-whole-line :wk "kill line"))
  ;; Region.
  (wal/colonel-sink
    "c" '(avy-copy-region :wk "copy region")
    "x" '(avy-kill-region :wk "delete region")
    "m" '(avy-move-region :wk "move region")
    "SPC" '(wal/avy-mark-region :wk "mark region")
    "w" '(avy-kill-ring-save-region :wk "kill region"))
  :commands (avy-with))
#+END_SRC
...
(use-package perspective
  ...
  (wal/captain
    "I" '(:keymap perspective-map :wk "perspective (map)"))
  (persp-mode +1)
...
(use-package projectile
  ...
  (wal/captain "P" '(:keymap projectile-command-map :wk "projectile (map)"))
  (projectile-mode +1)
  :custom
  ...
  (transient-define-prefix wal/org ()
    ...
    [["Commands"
     ...
     ["Visibility"
      ...
      ("v n" "imenu" imenu-list-smart-toggle)]
     ["Presentation"
...
(use-package org-agenda
  :config
  (advice-add 'org-agenda :filter-args
    #'wal/advise-org-agenda)
  :custom
...
(use-package verb
  ...
  (wal/captain :keymaps '(verb-mode-map verb-response-body-mode-map)
    "V" '(:keymap verb-command-map :wk "verb (commands)"))
  :bind
