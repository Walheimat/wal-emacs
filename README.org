...
There are a few nons-standard meta sequences.

+ =M-o= does =other-window=
...
(global-set-key (kbd "C-o") #'wal/open-line)         ; Overrides `open-line'.
(global-set-key (kbd "C-M-o") #'wal/open-line-above) ; Overrides `split-line'.
#+END_SRC
...
(use-package rg
  ...
  (wdb/pop-up "\\*rg\\*\\'")
  :custom
  (rg-keymap-prefix (wal/prefix-user-key "n"))
...
(declare-function projectile-project-buffers "projectile.el")
...
(use-package lsp-dart
  ...
  (wal/major
    ...
    "s" '(lsp-dart-dap-flutter-hot-restart :wk "restart")
...
(use-package java-mode
  :config
  (when wal/use-projectile
    (wal/major "t" '(wal/projectile-test-project :wk "test project")))
