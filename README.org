...
(defun wal/edge (scale-above scale-below)
  "Move the window edge using SCALE-ABOVE and SCALE-BELOW."
  (interactive)
  (let* ((other-buffer (and (next-window) (window-buffer (next-window))))
         (win (selected-window))
...
(defun wal/edge-horizontally (scale-left scale-right)
  "Move the window edge using SCALE-LEFT and SCALE-RIGHT."
  (interactive)
  (let* ((other-buffer (and (next-window) (window-buffer (next-window))))
         (win (selected-window))
...
(wal/general "et" '(wal/tangle-config :wk "tangle configuration"))
...
(global-set-key (kbd "C-H-h") #'wal/edge-left)
(global-set-key (kbd "C-H-l") #'wal/edge-right)
(global-set-key (kbd "C-H-j") #'wal/edge-down)
(global-set-key (kbd "C-H-k") #'wal/edge-up)
...
(use-package debug
  ...
  :ensure nil
  :config
  (wal/configure-help-buffer-display 'bottom))
#+END_SRC
...
(use-package elixir-mode
  ...
  (wal/major :keymaps 'elixir-mode-map
    "f" '(:ignore :wk "format")
    "fe" '(elixir-format :wk "elixir")
    "fl" '(wal/lsp-format :wk "lsp")))
#+END_SRC
...
(use-package python
  ...
  (wal/major :keymaps 'python-mode-map
    "f" '(:ignore :wk "format")
    "fl" '(wal/lsp-format :wk "format buffer")
...
(use-package blacken
  :general
  (wal/major :keymaps 'python-mode-map
    "fb" '(blacken-buffer :wk "blacken buffer")))
#+END_SRC
...
(defun wal/csharp-mode ()
  ...
  (message "Ouch!")
  (wal/disable/tabs)
  (wal/lsp) ; Run `lsp-install-server' to install.
  (flycheck-mode 1))
...
(use-package crux
  :general
  (wal/general
    "ei" '(crux-find-user-init-file :wk "open init file")
...
(use-package consult
  ...
  :general
  (wal/general
    "bf" '(consult-focus-lines :wk "focus lines")
  ...
  (wal/captain :keymaps 'flymake-mode-map
    "f" '(consult-flymake :wk "search error")))
...
(use-package counsel
  ...
  :general
  (wal/general
    "at" '(counsel-load-theme :wk "load theme")
