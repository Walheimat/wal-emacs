...
(use-package ef-themes
  :wal-ways nil
  :custom
  ;; Don't mess with the height of org headings!
  (ef-themes-height-0 1.0)
...
(defun wal/flycheck-file--callback (filename &optional defer-display)
  "Show FILENAME's errors in a distinct buffer.
  ...
  (lambda (_status &optional errors)
    (let* ((output-buffer (wal/flycheck-file--get-buffer)))
      (with-current-buffer output-buffer
        ...
        (goto-char (point-max))
        (insert (format "Checking file '%s':" filename))
        (if (null errors)
            (insert " no errors")
          (insert "\n"))
        (seq-do (lambda (err)
...
(defun wal/flycheck-file (file &optional defer-display)
      ...
      (let ((checker (flycheck-get-checker-for-buffer)))
        (if checker
            (let* ((check (flycheck-syntax-check-new
                   ...
                   (callback (wal/flycheck-file--callback (file-name-nondirectory file) defer-display)))
...
(defun wal/flycheck-packages ()
  "Run `wal/flycheck' on all package files."
  (interactive)
  (defvar projectile-enable-caching)
  (let ((output-buffer (wal/flycheck-file--get-buffer))
    ...
    (with-no-warnings
      (transient-define-prefix wal/config-dispatch ()
        ...
        [["Actions"
          ...
          ("f" "flycheck" wal/flycheck-packages)]
         ["Go to"
