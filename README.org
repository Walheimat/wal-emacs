...
(with-no-warnings
  (transient-define-prefix colonel ()
    ...
    [["Do"
     ...
     ["Check"
      ...
      ("c b" "cold boot" wal/perform-cold-boot)]
     ["Fluff"
  ...
  (with-eval-after-load 'which-key
    ...
    (which-key-add-key-based-replacements (wal/prefix-user-key "0") "zero-in")

    ;; This forces prefixing with `C-c w'.
    (when (not (display-graphic-p))
      (which-key-add-key-based-replacements "C-c w" "wal")))
...
(defun parallel-p ()
  "Check if we would trigger a `parallel' command."
  (equal 0 current-prefix-arg))
...
(use-package zone
  ...
  (declare-function zone-when-idle "ext:zone.el")
...
If no PREFIX is given, a slash is pre-pended to the first match."
  (let ((prefix (or prefix "")))
    ...
    (with-temp-buffer
      ...
      (concat
       prefix
       (if (match-string 1) (concat (match-string 1) ": ") "")
       (if (match-string 2) (match-string 2) "")))))

(defvar wal/popper-echo-regex-alist
  '(("\\*helpful \\(?1:variable\\|function\\|macro\\|command\\): \\(?2:[a-zA-Z-]+\\)\\*")
    ("\\* docker-compose \\(?1:up\\|run\\|build\\)\\( --rm\\)?\\ \\(?2:\\w+\\)" . "dc/")
    ("\\* docker container \\(?1:logs\\|stop\\)?\\ ?\\(--tail [[:digit:]]+\\)?\\ \\(?2:\\w+_\\w+_[[:digit:]]+\\)\\ \\*" . "dcon/")))

(defun wal/popper-echo-transform (buffer-name)
  "Transform BUFFER-NAME by truncating certain prefixes."
  (if-let ((match (cl-find-if (lambda (it) (string-match (car it) buffer-name)) wal/popper-echo-regex-alist)))
      (wal/popper-echo-replace (car match) buffer-name (cdr-safe match))
    buffer-name))
...
(declare-function parallel-p "wal-func.el")
...
(defun wal/consult--pre-narrow ()
  ...
  (when wal/consult-pre-narrow
    (pcase this-command
      ('consult-buffer
       (when (and (not (parallel-p))
                  (project-current nil))
         (setq unread-command-events (append unread-command-events (list ?p 32)))))

      ('wal/consult-project
       (when (and (consult--open-project-items)
                  (not (parallel-p)))
         (setq unread-command-events (append unread-command-events (list ?o 32))))))))
...
(use-package consult
  ...
  :init
  (general-define-key
    :prefix (wal/key-combo-for-leader 'wal/consult)
    :prefix-map 'wal/consult-map)
...
unles KEY is provided."
  (let ((function-name (intern (format "wal/project-%s" name)))
    ...
    `(progn
       ...
       (wal/major ,(format "p%s" key) ',function-name))))
...
(use-package project
  ...
  :general
  (wal/major "p" '(:ignore t :wk "project"))
...
#+BEGIN_SRC emacs-lisp
(use-package magit
  :commands (ma-magit magit-status)

  :init
  (harpoon magit-status-mode
    (setq-local transient-show-popup t))
...
(defun wal/prettier-mode-ignore-buffer-function ()
  "Ignore if it is a node_module buffer or a JSON buffer."
  (declare-function prettier--in-node-modules-p "ext:prettier.el")
...
(use-package prettier
  ...
  :custom
  (prettier-mode-ignore-buffer-function #'wal/prettier-mode-ignore-buffer-function))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
(defun wal/perform-cold-boot--on-success ()
  "Display success message."
  (message "Cold boot succeeded."))

(defun wal/perform-cold-boot--on-failure ()
  "Display failure message."
  (message "Cold boot failed."))

(defun wal/perform-cold-boot ()
  "Execute a cold boot."
  (interactive)

  (let* ((command "./cold-boot.sh")
