...
You can install either a =systemd= service using =wal/setup= or just
execute =emacs --daemon= in a terminal or your login shell's
configuration.
...
(use-package diff-mode
  :config
  (define-key diff-mode-shared-map (kbd "o") nil) ; Remove `diff-goto-source'.
  :ensure nil)
#+END_SRC
...
`universal-argument')."
  (interactive)
  (if-let* ((cont (and (not (equal current-prefix-arg '(4)))
                       (if wal/use-projectile
...
(use-package magit
  :config
  (transient-define-prefix wal/magit ()
...
(use-package flyspell
  ...
  (:map flyspell-mode-map
   ("C-." . ispell-word)) ; Replaces `flyspell-auto-correct-word'
                          ; which is still available via `C-M-i'.
  :general
