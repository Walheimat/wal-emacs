...
+ =C-c b= for =bookmark=, and if the modes are active:
+ =C-c f= for =flycheck=
...
(use-package eww
  :ensure nil
  :general
  (wal/general "ob" '(eww :wk "browse with eww")))
#+END_SRC
...
(use-package simple
  :ensure nil
  :general
  (wal/general "op" '(list-processes :wk "list processes"))
...
(defun wal/advise-aw--switch-buffer (&rest r)
  "Advise to use `wal/switch-buffer'."
  (wal/switch-buffer r))
...
#+BEGIN_SRC emacs-lisp
(defun wal/restclient-mode ()
  "Hook into `restclient-mode'."
  (ignore-errors
    (require 'jq-mode nil t)
    (require 'restclient-jq nil t) ; I put this in my load-path.
    (require 'cl nil t)))

(use-package restclient
  :hook (restclient-mode . wal/restclient-mode)
  :config
