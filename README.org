...
The rest of the BODY will be spliced into the lambda."
  ...
  (let* ((target (intern (concat (symbol-name name) "-hook")))
         ...
         (safe-body (cl-loop for (key val) on body by 'cddr
                             unless (memq key '(:messages :lsp :lsp-ignores :tabs :prog-like :captain :corfu :shallow :functions :treesit :ligatures))
                             collect key
                             and collect val))
    ...
    `(progn
       ,@(delete
          ...
          `((defun ,func-name ()
            ...
            ,(when ligatures
               (if ts-ready
                   `(wal/set-ligatures '(,name ,(intern ts-name)) ',ligatures)
                 `(wal/set-ligatures ',name ',ligatures)))

            (add-hook
             ',(if ts-ready
                   (intern (concat ts-name "-hook"))
                 target)
             ',func-name))))))
