...
(use-package ctrlf
  :config
  ;; Make additional functions cancel `ctrlf'.
  (eval
...
#+BEGIN_SRC emacs-lisp
(defun wal/orderfull-dispatch (word index total)
  "Fast dispatch taken from `corfu' docs.

Check if WORD length is smaller than 4, INDEX is 0 and TOTAL is 1."
  (and (= index 0) (= total 1) (length< word 4)
       `(orderless-regexp . ,(concat "^" (regexp-quote word)))))

(use-package orderless
  :config
  ;; Register a new completion style (used by `lsp-mode').
  (orderless-define-completion-style orderfull
    (orderless-style-dispatchers '(wal/orderfull-dispatch))
...
#+BEGIN_SRC emacs-lisp
(defun wal/pick-default-agenda (&rest _args)
  "Advise `org-agenda' to just display current day."
  (list (wal/univ-p) "a"))

(defun wal/switch-to-tasks ()
  "Go to task directory with Dired."
  (interactive)
  (dired wal/agenda-tasks-directory))

(use-package org-agenda
  :config
  ;; No need for the dispatch.
  (advice-add
   'org-agenda :filter-args
   #'wal/pick-default-agenda)
  :custom
...
#+BEGIN_SRC emacs-lisp
(defun wal/configure-lsp-pylsp ()
  "Configure `lsp-mode' python variables."
...
(use-package pyvenv
  :wal-ways nil
  :config
  (add-hook 'pyvenv-post-activate-hooks #'wal/configure-lsp-pylsp)
  :ensure nil)
#+END_SRC
