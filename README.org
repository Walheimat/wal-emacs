...
+ =H-m= for =multiple-cursors= (where =m= selects all like this)
...
(global-set-key (kbd "H-[") 'wal/text-scale/body)
...
Configurations for other included packages.
...
(use-package multiple-cursors
  ...
  :general
  (wal/mc-leader
    "m" '(mc/mark-all-like-this :wk "all like this")
...
(use-package avy
  ...
  (wal/avy-leader
    ...
    "c" '(avy-goto-char :wk "char")
    "k" '(avy-kill-whole-line :wk "kill")
    "m" '(avy-move-line :wk "move")
    "w" '(avy-copy-line :wk "copy")
    "d" '(avy-goto-word-0 :wk "word")
    "b" '(wal/avy-goto-line-and-char :wk "go to line and char")
    "r" '(:ignore t :wk "region")
    "rw" '(avy-copy-region :wk "copy")
...
(defun wal/advise-selectrum(func &rest r)
  ...
  (let ((completing-read-function #'selectrum-completing-read)
         ...
         ;; Cannot advise `dired-read-dir-and-switches' and
         ;; `minibuffer-message'.
...
(general-create-definer wal/perspective-leader
  ...
  :prefix-command 'wal/perspective
...
You will have to install them yourself[fn:12].
