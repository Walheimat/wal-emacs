...
It's also used for two command maps:

+ =H-l= for =goto-map=
+ =H-u= for a custom =consult= map.
...
+ =i= for =iedit-mode=
+ =m= to move a line
+ =s= to open the scratch buffer
+ =.= to mark all "like this"
+ =u= for additional =consult= actions
+ =w= to kill a line
+ =x= to delete a line.
...
+ =h= to mark a region
+ =m= to move a region
+ =s= to create new scratch buffer using =crux=
...
(defun wal/tempel-setup-capf ()
  "Make sure `tempel-complete' runs first."
...
(use-package crux
  ...
  (wal/lieutenant! "s" 'wal/scratch-buffer 'crux-create-scratch-buffer))
#+END_SRC
...
(use-package consult
  ...
  (general-define-key
   :keymaps 'wal/consult-map
   :wk-full-keys nil
   "s" '(wal/consult-line :wk "line")
   "c" '(wal/consult-clock-in :wk "clock in")
   "l" '(consult-locate :wk "locate")
   "r" '(consult-register :wk "register")
   "f" '(consult-recent-file :wk "recent file"))
  ...
  (wal/lieutenant "u" '(:keymap wal/consult-map :wk "consult")))
#+END_SRC
