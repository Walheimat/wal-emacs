...
User-reserved combinations are used for certain dispatches:
...
(when wal/use-hyper-prefix
  (general-define-key
   (wal/prefix-user-key "u") 'universal-argument))
  ...
  (cond
   ((string-match "\\*helpful" buffer-name)
    (concat
     ...
     "/help"))
   ((string-match "\\* docker-compose \\(up\\|run\\|build\\)\\( --rm\\)?" buffer-name)
    (concat
     (substring
      (replace-regexp-in-string
       "\\* docker-compose \\(up\\|run\\|build\\)\\( --rm\\)?"
       "" buffer-name)
      0 -2)
     "/dc"))
   (t buffer-name)))
