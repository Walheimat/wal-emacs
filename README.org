...
(setq mouse-yank-at-point t
      ...
      read-process-output-max (wal/bytes-per-mb 2)
      ...
      ;; Undo limits.
      undo-limit (* 160000 2)
      undo-strong-limit (* 240000 2)
      undo-outer-limit (* 24000000 2))
#+END_SRC
...
(defvar wal/org-tree-slide-visual-fill-column-width 180
  "The `visual-fill-column-width' for `org-tree-slide' presentations.")

(defun wal/org-tree-slide-calculate-relative-visual-fill-column-width ()
  "Calculate a `visual-fill-column-width' relative to text scaling.
Scaling tries to accommodate positive and negative scaling."
  (let* ((factor (if (and (boundp 'text-scale-mode-amount)
                          (not (zerop text-scale-mode-amount)))
                     text-scale-mode-amount
                   1))
         (func (cond ((> factor 0) '(:operator / :convert ceiling))
                     (t '(:operator * :convert floor)))))
    (abs (apply (plist-get func :convert)
                (list (apply (plist-get func :operator)
                             (list wal/org-tree-slide-visual-fill-column-width
                                   (* factor text-scale-mode-step))))))))

(defun wal/org-tree-slide-play ()
  "Make presentable."
  (setq visual-fill-column-width (wal/org-tree-slide-calculate-relative-visual-fill-column-width)
