...
+ =H-i= loads registers using =consult= (stores when called with =C-u=)
+ =H-j= switches buffer
+ =H-k= acts on the current point with =embark= (immediately if called
  with =C-u=)
+ =H-l= to jump to line with =avy= (beginning of line if called with =C-u=)
...
+ =!= to toggle =read-only-mode= for buffer
+ =t= to scale text
...
(wal/on-boot key-bindings
  ...
  (wal/general-sink "!" 'read-only-mode)
...
(defun wal/kill-some-file-buffers ()
  ...
  (interactive)
  (let ((file-buffers (seq-filter (lambda (it) (buffer-file-name it)) (buffer-list))))
...
#+BEGIN_SRC emacs-lisp
(defun wal/scratch-buffer (&optional create)
  "Switch to the scratch buffer.

If CREATE is t, do and use that instead."
  (interactive "P")
  (let ((buf (if create
...
(cl-defmacro wal/hook (name docs &body body &key messages lsp lsp-ignores tabs prog-like captain corfu &allow-other-keys)
...
The rest of the BODY will be spliced into the lambda."
  ...
  (let ((target (intern (concat (symbol-name name) "-hook")))
        (safe-body (cl-loop for (key val) on body by 'cddr
                            unless (memq key '(:messages :lsp :lsp-ignores :tabs :prog-like :captain :corfu))
                            collect key
                            and collect val))
...
#+BEGIN_SRC emacs-lisp
(defvar wal/scratch-persist-file nil)
...
(defun wal/select-fixed-font (font)
  ...
  (interactive
   (list
    (completing-read
     (format "Select fixed font (current: %s): " (face-attribute 'fixed-pitch :family))
     (wal/available-fonts wal/fixed-fonts))))
...
(defun wal/set-fixed-font-height (&optional height)
  ...
  (interactive
   (list
    (read-number
     (format "Set fixed font height (currently: %s): " wal/fixed-font-height))))
...
(defun wal/select-variable-font (font)
  ...
  (interactive
   (list
    (completing-read
     (format "Select variable font (current: %s): " (face-attribute 'variable-pitch :family))
     (wal/available-fonts wal/variable-fonts))))
...
(defun wal/set-variable-font-height (&optional height)
  ...
  (interactive
   (list
    (read-number
     (format "Set variable font height (currently: %s): " wal/variable-font-height))))
  (let ((sensible-height (max (min height 300) 80)))
...
"
  ...
  :general
  (wal/general-sink "w" '(wal/winner/body :wk "winner")))
#+END_SRC
...
(use-package dictionary
  ...
  :general
  (wal/general-sink "d" '(dictionary-lookup-definition :wk "dict")))
#+END_SRC
...
(defun wal/before-mc ()
  "Disable modes for `multiple-cursors-mode'."
  (when (bound-and-true-p lispy-mode)
    (lispy-mode -1)
    (add-to-list 'wal/mc-disabled 'lispy))

  (when corfu-mode
    (add-to-list 'wal/mc-disabled 'corfu)
    (corfu-mode -1)))

(defun wal/after-mc ()
  "Toggle modes disabled by `mutliple-cursors-mode'."
  (when (memq 'lispy wal/mc-disabled)
    (lispy-mode 1))

  (when (memq 'corfu wal/mc-disabled)
    (corfu-mode 1))

  (setq-local wal/mc-disabled nil))
...
(defun wal/popper--spared-p (buffer)
  ...
  (let* ((name (buffer-name buffer))
    ...
    (message "Checking if %s should be spared" name)
    (when spared (message "Spared %s" name))))
...
(defcustom wal/consult-key "["
  "The key used for `consult'."
  :type 'key-sequence
  :group 'wal-complete)
#+END_SRC
...
(use-package corfu
  :hook
  (((org-mode prog-mode prog-like) . corfu-mode)
...
(use-package consult
  ...
  (bind-key (wal/prefix-user-key wal/consult-key)
            #'(lambda nil
                (interactive)
                (use-package-autoload-keymap 'wal/consult-map 'consult nil)))

  (bind-key wal/consult-key 'consult-goto-line 'goto-map)
  ...
  (with-eval-after-load 'rg-menu
    (transient-append-suffix 'rg-menu "p" `(,wal/consult-key "Using consult" consult-ripgrep)))
  (with-eval-after-load 'lsp-mode
    (transient-append-suffix 'wal/lsp-dispatch '(0 3 1)
      `(,wal/consult-key "symbol" consult-lsp-file-symbols)))
  (with-eval-after-load 'org
    (transient-append-suffix 'wal/org-mode-dispatch "o"
      `(,wal/consult-key "go to heading" consult-org-heading)))

  (which-key-add-key-based-replacements
    (wal/prefix-user-key wal/consult-key)
    '(nil . "consult"))
...
(use-package eshell
  ...
  :general
  (wal/general "xe" '(eshell :wk "eshell")))
#+END_SRC
...
(use-package consult-flycheck
  ...
  (:map flycheck-command-map
   ("b" . flycheck-buffer)) ;; Uses c.

  :general
  (general-define-key
    :keymaps 'flycheck-command-map
    "c" '(consult-flycheck :wk "consult flycheck")))
#+END_SRC
...
(defvar wal/lsp-corfu-values '(0.1 0))
...
minimum prefixes are reduced using CORFU-VALUES a list of (DELAY
PREFIX-LENGTH)."
  (unless (wal/slow-lsp-p major-mode)
    (let* ((values (or corfu-values wal/lsp-corfu-values))
           (delay (car values))
           (prefix (cadr values)))
...
(defun wal/check-coverage--calculate-coverage ()
  ...
  (when-let* ((buf (seq-find (lambda (it) (string-match "-async\\*" (buffer-name it))) (buffer-list)))
    ...
    (format "%s%%" (/ (apply '+ nums) (length nums)))))
