...
(cl-defmacro wal/hook
    (name
     docs
     &body body
     &key messages lsp lsp-ignores tabs prog-like captain corfu shallow functions treesit
     &allow-other-keys)
...
The rest of the BODY will be spliced into the lambda."
  ...
  (let* ((target (intern (concat (symbol-name name) "-hook")))
         ...
         (ts-name (pcase treesit
                    ('base "base")
                    (_ "ts")))
         (ts-target (thread-first
                     name
                     (symbol-name)
                     (split-string "-")
                     (cl-first)
                     (concat "-" ts-name "-mode-hook")
                     (intern)))
        (safe-body (cl-loop for (key val) on body by 'cddr
                            unless (memq key '(:messages :lsp :lsp-ignores :tabs :prog-like :captain :corfu :shallow :functions :treesit))
                            collect key
                            and collect val))
    ...
    `(progn
       ,@(delete
          ...
          `((defun ,func-name ()
            ...
            (add-hook
             ',(if (and treesit (wal/modern-emacs-p 29)) ts-target target)
             ',func-name))))))
...
(use-package python
  ...
  (wal/set-ligatures 'python-mode '("->" "__"))
...
(use-package lisp-mode
  ...
  (wal/set-ligatures 'lisp-data-mode '(";;")))
#+END_SRC
...
(use-package clojure-mode
  ...
  (wal/set-ligatures 'clojure-mode '(";;" "->" "->>")))
#+end_src
