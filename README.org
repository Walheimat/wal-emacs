...
I use many[fn:8] custom keybindings. You can find an overview [[file:docs/bindings.org][here]].
...
several so-called expansion packs[fn:9].
...
#+BEGIN_SRC emacs-lisp
(defvar wal/mc-conflicting-modes '(lispy-mode corfu-mode)
...
You need to install the icons yourself[fn:10].
...
=vterm= is better[fn:12] than included shells.
...
Takes care[fn:13] of all your virtual environment needs.
...
name are deleted beforehand."
  ...
  (let ((buffer-name "\\*wal-async\\*")
        (full-command (concat "cd " wal/emacs-config-default-path " && " command)))
...
[fn:9] Alternate spelling is "expansion pak".

[fn:10] The =all-the-icons= icons need to be downloaded manually by
...
[fn:11] You might have to call =emojify-download-emoji= to download a
set that supports your emojis.

[fn:12] If you're on an older distro your =libvterm= package might be
too old.

[fn:13] Currently requires [[https://daseldocs.tomwright.me/installation#manual ][dasel]].

[fn:14] Binding reference [[http://oremacs.com/lispy/][here]].
