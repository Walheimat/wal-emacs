...
(defun wal/before-mc ()
  ...
  (when corfu-auto
    (add-to-list 'wal/mc-disabled 'corfu-auto)
...
(defun wal/after-mc ()
  ...
  (when (memq 'corfu-auto wal/mc-disabled)
...
(defun wal/tempel-setup-capf ()
  "Make sure `tempel-complete' is run first."
  (let* ((current completion-at-point-functions)
         (prepended (cons #'tempel-complete current))
         (final (delete-dups prepended)))

     (setq-local completion-at-point-functions final)))
...
PREFIX-LENGTH)."
  (unless (wal/slow-lsp-p major-mode)
    (let* ((values (or corfu-values (list 0.0 0)))
           (delay (car values))
           (prefix (cadr values)))
