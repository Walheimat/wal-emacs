...
(defun wal/load-active-theme ()
  ...
  (when wal/active-theme
    (if (custom-theme-p wal/active-theme)
...
(wal/define-init-setup visuals
  ...
  :always
  ((wal/transparency)
   ;; Load `modus-themes' again because of customization quirk.
   (when (memq wal/active-theme '(modus-operandi modus-vivendi))
     (wal/load-active-theme)))
  :immediately t)
...
Busier and prettier[fn:15] =modeline=.
...
(use-package projectile
  ...
  (wal/captain
    "h" '(:keymap projectile-command-map :wk "projectile"))
  (projectile-mode +1)
  :custom
...
(defun wal/org-tree-slide-stop ()
  ...
  (when (fboundp 'outline-show-all)
    (outline-show-all))
  (when (fboundp 'org-sticky-header-mode)
    (org-sticky-header-mode +1))
  (mixed-pitch-mode -1))
...
(wal/define-expansion-pack python
  ...
  :packages (python-isort python-pytest pyvenv poetry)
  :extras (ein))
#+END_SRC
...
(use-package lsp-mode
  ...
  (wal/captain :keymaps 'lsp-mode-map
    "p" '(:keymap lsp-command-map :wk "lsp"))
  ...
  (lsp-keymap-prefix (wal/prefix-user-key (concat wal/captain-key " " "p")))
...
#+BEGIN_SRC emacs-lisp
(defun wal/kubernetes-utils--save-window-state (&rest body)
  "Restore window state after executing BODY.

This is an exact copy of the original function which was defined
as a macro (which does not work for me)."
  (let ((pos (point))
        (col (current-column))
...
(use-package kubernetes
  ...
  :wal-ways nil
  :init
  ;; This already has an autoload that doesn't load the base package.
  (autoload #'kubernetes-overview "kubernetes" nil t)
  :config
  (defalias 'kubernetes-utils--save-window-state 'wal/kubernetes-utils--save-window-state)
  :custom
