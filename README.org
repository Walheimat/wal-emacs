...
#+BEGIN_SRC emacs-lisp
(defvar wal/mc-conflicting-modes '(paredit-mode corfu-mode)
...
#+BEGIN_SRC emacs-lisp
(defun wal/avy-goto-word (&optional in-line)
  "Jump to a word.

If IN-LINE is t, jump to word in line in the current window."
  (interactive "P")
...
(use-package project
  ...
  (project-vc-extra-root-markers '("pom.xml" "package.json"))
...
(defun wal/dired-buffer-p (buffer)
  "Check if buffer is a dired buffer."
  (with-current-buffer buffer
...
(defun wal/check-coverage--calculate-coverage ()
  ...
  (when-let* ((buffers (seq-filter (lambda (it) (string-match "\\*wal-async" (buffer-name it))) (buffer-list)))
              (compare (lambda (a b)
                         (> (string-to-number (substring (buffer-name a) -2 -1))
                            (string-to-number (substring (buffer-name b) -2 -1)))))
              (latest (car (seq-sort compare buffers)))
              (matches (with-current-buffer latest
