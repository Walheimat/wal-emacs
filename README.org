...
(with-no-warnings
  ...
  (administrator "n" 'display-line-numbers-mode)
...
(use-package hideshow
  ...
  :general
  (editor :keymaps 'hs-minor-mode-map
    "h" 'hs-toggle-hiding))
#+END_SRC
...
(use-package dictionary
  ...
  :general
  (administrator "d" 'dictionary-lookup-definition))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
(defvar-local wal/org-capture-tasks "tasks.org")
(defvar-local wal/org-capture-tasks-heading "Tasks")

(defun wal/org-capture-find-tasks ()
  "Find (a project's) tasks file.

If the project root cannot be determined, use `org-directory'. as
destination."
  (let ((proj-root (when-let ((current (project-current)))
                     (project-root current))))

    (expand-file-name wal/org-capture-tasks (or proj-root org-directory))))

(defun wal/org-capture-find-location ()
  "Find the location in the file."
  (when-let ((pos (or (org-find-exact-headline-in-buffer wal/org-capture-tasks-heading nil t)
                      (point-max))))
    (goto-char pos)))
#+END_SRC
  ...
  (org-capture-templates
   `(("c" "current task" plain
     ...
     ("p" "project task" entry
      (file+function wal/org-capture-find-tasks wal/org-capture-find-location)
      "* TODO %?\n\n%F\n\n%i"
      :empty-lines-before 1)))
