...
several so-called expansion packs[fn:14].
...
This is a combination of configurations for built-in packages[fn:15]
...
Protesilaos' super configurable[fn:16] themes.
...
You need to install the icons yourself[fn:17].
...
=vterm= is better[fn:19] than included shells.
...
Takes care[fn:21] of all your virtual environment needs.
...
(defmacro use-package (name &rest args)
  "Message what you would have done."
  `(message "Would have loaded %s" ',name))
#+END_SRC
...
[fn:13] You can find the code coverage [[https://coveralls.io/github/Walheimat/emacs-config][here]].

[fn:14] Alternate spelling is "expansion pak".

[fn:15] The following packages (and their related or sub-packages) are
...
[fn:16] Unfortunately these themes require to the customizations to be
set before loading, which means that when using these themes you have
to load them twice.

[fn:17] The =all-the-icons= icons need to be downloaded manually by
...
[fn:18] You might have to call =emojify-download-emoji= to download a
set that supports your emojis.

[fn:19] If you're on an older distro your =libvterm= package might be
too old.

[fn:20] None of the language packages are ensured, meaning you will
need to install them yourself.

[fn:21] Currently requires [[https://daseldocs.tomwright.me/installation#manual ][dasel]].

[fn:22] Binding reference [[http://oremacs.com/lispy/][here]].

[fn:23] All languages listed [[https://emacs-lsp.github.io/lsp-mode/page/languages/][here]].
