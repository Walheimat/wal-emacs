...
+ copy the init file included in this repo by running =cp ~/.emacs.d/emacs-config/templates/.emacs ~=
...
you will need to adapt the variable =wal/emacs-config-default-path= in the example init file you just copied.
...
[fn:4] This includes this very config which will be exported as
a package collection.

This config uses the =all-the-icons= package
