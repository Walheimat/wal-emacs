...
Some leader keys have so-called sinks for additional commands. The
sink for =captain= toggles minor modes.
...
+ =d= to duplicate lines with =crux=
+ =q= and =Q= to =query-replace=
+ =v= for =vundo= (visual undo).

Where possible, they are grouped by package or concern. The prefixes
are:
...
(defvar wal/leader-with-sink '(wal/general wal/captain wal/lieutenant))
