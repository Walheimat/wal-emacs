...
(cl-defmacro wal/hook (name docs &body body &key messages lsp lsp-ignores tabs prog-like lieutenant before-save &allow-other-keys)
...
BEFORE-SAVE is a list of functions to add to `before-save-hook'.
...
The rest of the BODY will be spliced into the lambda."
  ...
  (let ((target (intern (concat (symbol-name name) "-hook")))
        (safe-body (cl-loop for (key val) on body by 'cddr
                            unless (memq key '(:messages :lsp :lsp-ignores :tabs :prog-like :lieutenant :before-save))
                            collect key
                            and collect val))
          ...
          `((defun ,func-name ()
              ...
              ,@(delete
                 ...
                 `(,(when messages
                   ...
                   ,@(when before-save
                       (seq-map (lambda (it) `(add-hook 'before-save-hook ',it nil t)) before-save))
                   ,(when lieutenant
...
(ert-deftest test-wal/hook--before-save ()
  (match-expansion
   (wal/hook test
     "We're just testing."
     :messages '("Just testing")
     :before-save (test-buffer really-test)
     (message "hi"))
   `(progn
      (defun wal/test-hook ()
        "We're just testing."
        ...
        (wal/disable-tabs)
        (message "hi")
        (add-hook 'before-save-hook 'test-buffer nil t)
        (add-hook 'before-save-hook 'really-test nil t))
      (add-hook 'test-hook 'wal/test-hook))))
#+END_SRC
...
(use-package project
  ...
  :config
  (wal/captain "p" '(:keymap project-prefix-map :wk "project"))
...
Optionally, a buffer display action can be passed of the
...
(use-package js2-mode
  ...
  (transient-define-prefix wal/js2-mode-dispatch ()
    "Access JS commands."
    [["Actions"
      ("b" "bounce" js2-indent-bounce)]])
  ...
  :bind
  (:map js2-mode-map
   ;; `js2-bounce-indent-p' also binds <TAB>.
   ("<backtab>" . js2-indent-bounce)))
#+END_SRC
...
(use-package prettier
  ...
  :config
  (transient-append-suffix 'wal/js2-mode-dispatch '(0 0 0)
...
(defun wal/lsp-auto-prefix ()
  "Get the desired prefix length for `corfu-auto-prefix'.

This is 0 except for modes derived from `js2-mode'."
  (if (derived-mode-p 'js2-mode) 3 0))
...
minimum prefixes are reduced."
  (unless (wal/slow-lsp-p major-mode)
    (setq-local corfu-auto-delay 0.0
                corfu-auto-prefix (wal/lsp-auto-prefix)
...
(use-package lsp-mode
  ...
  (with-eval-after-load 'js2-mode
    (transient-append-suffix 'wal/js2-mode-dispatch '(0 0 -1)
