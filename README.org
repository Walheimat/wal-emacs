...
Both functions will be called interactively."
  ...
  (let ((a-name (symbol-name a))
    ...
    `(defun ,(intern (concat "wal/univ-" a-name)) (&optional call-other)
       ,(format "Call `%s' unless `universal-argument' is present, in that case CALL-OTHER `%s' instead."
                a-name
                b-name)
       (interactive "P")
       (if call-other
...
(use-package embark
  :init
  (wal/univ embark-dwim embark-act)
  :config
  (wal/configure-embark)
  :custom
  (embark-mixed-indicator-delay 2.0)
  :wal-bind
  ("k" . wal/univ-embark-dwim)
  :after vertico
  :commands (wal/univ-embark-dwim))
