...
(defun use-package-handler/:wal-ways (name _keyword pred rest state)
  "Update state and only expand conditionallly.

The conditions are: (1) the PRED is t, (2) `wal/minimal' is nil
or (3) NAME is never excluded.

The STATE is updated as well before parsing REST so we can
further prevent ELPA installations."
  (when (and (fboundp 'use-package-process-keywords)
    ...
    (let* ((active (or pred
           ...
           (body (use-package-process-keywords
                   name
                   rest
                   (use-package-plist-maybe-put state :do-install active))))
      `((when ,active
          ,@body)))))
