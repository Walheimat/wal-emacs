...
(defun wal/general-create-leader-sink (key)
  "Create a sink prefix for the given KEY symbol."
  (let* ((symv (symbol-value key))
         (suff (substring symv -1))
         (symn (symbol-name key))
         (wk (upcase (concat (substring symn 4 -4) "!"))))
         (general-define-key :prefix symv
                             suff
                             `(:ignore t :wk ,wk))))
(use-package general
  :defer t
  ...
  (wal/general-create-leader-sink 'wal/lieutenant-key))
#+END_SRC
...
"
  ...
  ("g" balance-windows "balance")
  ("q" nil "cancel"))

(defhydra wal/scroll-other-window (:timeout 1 :hint nil)
  "
Scroll other window u_p_/dow_n_.
"
...
(use-package hl-todo
  ...
  :general
  (wal/lieutenant "/t" '(hl-todo-insert :wk "insert todo")))
#+END_SRC
...
(use-package counsel
  ...
  (wal/lieutenant
    ...
    "ob" '(counsel-bookmark :wk "bookmark")
