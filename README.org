...
(ert-deftest test-wal/disable-tabs--disables ()
  (with-temp-buffer
    ...
    (with-mock hack-local-variables (lambda () 'hacked)
      (should (equal 'hacked (wal/disable-tabs))))))
...
(ert-deftest test-wal/async-process ()
  (wal/async-process "echo \"the world is a vampire\""
    (lambda ()
      (with-current-buffer (get-buffer-create "*wal-async*")
        (should (string-match "the world is a vampire" (buffer-string)))))
    (lambda (_) nil)))
#+END_SRC
