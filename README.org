...
It also provides an alternative bindings for command maps:
...
It gives access to minor-mode[fn:11] commands and commands maps that are
...
#+BEGIN_SRC emacs-lisp
(defun wal/async-process (command callback error-callback &optional suffix)
  "Start async process COMMAND with CALLBACK and ERROR-CALLBACK.

Optionally, the buffer is created with SUFFIX."
  (with-current-buffer (compilation-start
                        command t
                        (lambda (&rest _)
                          (if suffix
                              (generate-new-buffer-name (concat "*wal-async-" suffix "*"))
...
(use-package register
  ...
  :config
  (wal/lieutenant "r" '(:keymap ctl-x-r-map :wk "register")))
#+END_SRC
...
(use-package perspective
  ...
  (with-eval-after-load 'consult
    ...
    (defvar wal/consult-source-dired
    ...
    (wal/insert-after
     ...
     'consult--source-buffer
     wal/consult-source-dired))

  ;; Register command map.
  (wal/captain "i" '(:keymap perspective-map :wk "perspective"))
...
(use-package projectile
  ...
  ;; Register command map.
  (wal/captain "p" '(:keymap projectile-command-map :wk "projectile"))
...
Attempts to restore view unless NO-RESTORE is t."
  ...
  (let ((wconf wal/org-agenda-window-configuration))
    (if (and wconf (not no-restore))
        (condition-case nil
            (set-window-configuration wconf)
          (error
           (setq wal/org-agenda-window-configuration nil)
           (message "Couldn't restore agenda windows, please try again!")))
...
(use-package org-roam
  ...
  (transient-define-suffix wal/org-roam-maybe-ui ()
    "Run `org-roam-ui' if it is bound."
    :key "u"
    :description "ui"
    :inapt-if-not (lambda () (fboundp 'org-roam-ui-mode))
    (interactive)
...
#+BEGIN_SRC emacs-lisp
(use-package magit
  :commands (wal/univ-magit-status wal/magit-dispatch)

  :init
  (wal/univ magit-status wal/magit-dispatch)

  :config
  (transient-define-suffix wal/magit-dispatch-maybe-timemachine ()
    "Maybe run `git-timemachine-toggle' if it is bound."
    :key "t"
    :description "time machine"
    :inapt-if-not (lambda () (fboundp 'git-timemachine-toggle))
    (interactive)
  ...
  (transient-define-prefix wal/magit-dispatch ()
    ...
    [["Core"
     ...
     ["Other"
      ("x" "run" magit-run)
      ("c" "clone" magit-clone)
      (wal/magit-dispatch-maybe-timemachine)]])
...
(defcustom wal/prefer-rjsx nil
  "Wheter to prefer `rjsx-mode' over plain `js2-mode' for normal JavaScript files."
  :group 'wal-lang
  :type 'boolean)
#+END_SRC
...
(use-package python
  ...
  (transient-define-suffix wal/python-maybe-poetry ()
    "Run poetry if it exists."
    :key "p"
    :description "poetry"
    :inapt-if-not (lambda () (fboundp 'poetry))
    (interactive)
    (call-interactively 'poetry))

  (transient-define-suffix wal/python-maybe-pyvenv-activate ()
    "Run `pyvenv-activate' if it exists."
    :key "v"
    :description "activate pyvenv"
    :inapt-if-not (lambda () (fboundp 'pyvenv-activate))
    (interactive)
    (call-interactively 'pyvenv-activate))

  (transient-define-suffix wal/python-maybe-ein:run ()
    "Run `ein:run if it exists."
    :key "e"
    :description "ein start"
    :inapt-if-not (lambda () (fboundp 'ein:run))
    (interactive)
  ...
  (transient-define-prefix wal/python ()
    ...
    [["Inferior shell"
     ...
     ["Extensions"
      (wal/python-maybe-poetry)
      (wal/python-maybe-pyvenv-activate)
      (wal/python-maybe-ein:run)]])
...
(use-package js2-mode
  ...
  (with-eval-after-load 'prettier
    (transient-define-suffix wal/js-lsp-rename-file ()
      "Call `lsp-javascript-rename-file' if possible."
      :key "r"
      :description "rename file"
      :inapt-if-not (lambda () (fboundp 'lsp-javascript-rename-file))
      (interactive)
      (call-interactively 'lsp-javascript-rename-file))

    (transient-define-prefix wal/js ()
      "Access JS commands."
      [["Actions"
        ("p" "prettier" prettier-prettify)
        (wal/js-lsp-rename-file)]]))
...
#+BEGIN_SRC emacs-lisp
(defun wal/maybe-use-rjsx ()
  "If `rjsx-mode' is preferred, safely switch to."
  (hack-local-variables)

  (when wal/prefer-rjsx
    (defvar js2-mode-hook)
    (let ((js2-mode-hook nil))
...
(use-package lsp-mode
  ...
  (transient-define-suffix wal/lsp-dispatch-maybe-consult-lsp ()
    "Only run `consult-lsp' if it is bound."
    :description "symbol"
    :key "s"
    :inapt-if-not (lambda () (fboundp 'consult-lsp-file-symbols))
    (interactive)
  ...
  (transient-define-prefix wal/lsp-dispatch ()
    ...
    [["Format"
     ...
     ["Go to"
      ...
      (wal/lsp-dispatch-maybe-consult-lsp)
...
(defun wal/check-coverage ()
  ...
  (let* ((suffix "coverage")
         (dir wal/emacs-config-package-path)
         (command (format "cd %s && cask exec ert-runner && cat coverage.txt" dir)))

    (dolist (buf (buffer-list))
      (when (string-match "-coverage\\*" (buffer-name buf))
        (when-let ((window (get-buffer-window buf)))
          (delete-window window))
        (kill-buffer buf)))
