...
+ =H-j= finds project file using =counsel-projectile=
+ =H-k= searches with =ag= in project
+ =H-l= jumps to line (and =H-o= to word in line) with =avy=
+ =H-n= switches windows using =crux=
...
+ =H-SPC= switches to buffer using =counsel-projectile=
...
(use-package crux
  :bind
  (("C-x o" . crux-other-window-or-switch-buffer) ;; Overrides `other-window'.
  ...
  (("9" . wal/crux-duplicate-current-line-or-region)
   ...
   ("n" . crux-other-window-or-switch-buffer))
  :general
  (wal/lieutenant
    "x" '(:ignore t :wk "crux")
...
(use-package perspective
  ...
  (:map persp-mode-map
   ...
   ("C-x x k" . all-the-icons-ivy-rich-kill-buffer)
