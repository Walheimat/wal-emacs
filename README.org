...
(defcustom wal/maximize-by-default t
...
(setq use-dialog-box nil                     ; Never use dialog boxes.
      global-auto-revert-non-file-buffers t) ; Update Dired buffers.
#+END_SRC
...
+ =H-m= dispatches =wal/magit-dispatch= (if called with =C-u=)
...
*** Differentiating by =universal-argument=
...
(cl-defmacro wal/univ (a b &key pass-args)
  "Define a `universal-argument'-modified function for A.
It will call B instead if the argument is present.

If PASS-ARGS is non-nil, both functions will be called
interactively providing an empty list as arguments."
  (declare (indent defun))
  (let ((a-name (symbol-name a))
        (b-name (symbol-name b)))
    `(defun ,(intern (concat "wal/univ-" a-name)) ()
       ,(format "Call `%s' unless `universal-argument' is present, in that case call `%s' instead."
                a-name
                b-name)
       (interactive)
       (if (wal/univ-p)
           ,(if pass-args `(funcall-interactively ',b '()) `(,b))
         ,(if pass-args `(funcall-interactively ',a '()) `(,a))))))
#+END_SRC
...
(cl-defmacro wal/define-expansion-pack (name &key packages extras)
  "Define an expansion pack of PACKAGES under NAME."
  (declare (indent defun))
  `(add-to-list 'wal/expansion-packs '(,name . (:packages ,packages :extras ,extras))))
...
(declare-function wal/setup-p "wal.el")
...
(if (daemonp)
    (progn
      (wal/setup-visuals)
      (add-hook 'server-after-make-frame-hook #'wal/transparency))
...
(defun wal/font-update (attribute value faces &optional refresh)
  "Set ATTRIBUTE to VALUE for FACES.

Optionally REFRESH font width cache aftewards."
  (mapc (lambda (it)
    (when (internal-lisp-face-p it)
      (set-face-attribute it nil attribute value)))
    faces)
  (when (and refresh (boundp 'doom-modeline-mode) doom-modeline-mode)
...
(defun wal/select-fixed-font (font)
  ...
  (wal/font-update :font font '(default fixed-pitch) t))
...
(defun wal/set-fixed-font-height (&optional height)
  ...
  (let ((sensible-height (max (min height 200) 80)))
    (wal/font-update :height sensible-height '(default fixed-pitch) t)
...
`variable-pitch' using `wal/variable-fonts'."
  ...
  (when (and (featurep 'doom-modeline) doom-modeline-mode)
    (run-with-idle-timer 0.2 nil #'doom-modeline-refresh-font-width-cache)))
...
(defun wal/dead-shell-p ()
  ...
  (let ((buf (current-buffer)))
    (with-current-buffer buf
      (and (or (derived-mode-p 'shell-mode)
...
(defcustom wal/cap-provider 'company
...
(use-package consult
  ...
  (wal/colonel
    ...
    "ug" '(consult-global-mark :wk "global mark"))
  (wal/colonel-sink
    "n" '(consult-ripgrep :wk "rg")
    "i" '(wal/consult-ripgrep-ignored :wk "ignored rg"))
...
(defun wal/advise-persp-format-name (&rest args)
  ...
  (let* ((name (car args))
    ...
    (if (and (> combined-length wal/perspective-max-modeline-length)
        ...
        (substring name 0 wal/perspective-truncated-length)
      name)))
...
(use-package perspective
  ...
  :init
  (wal/univ persp-switch persp-buffer-menu :pass-args t)
  :config
...
;; Allow including `wal/define-expansion-pack' macro.
(eval-when-compile
...
(use-package org-roam
  ...
  (wal/sergeant
    ...
    "<SPC>" '(org-roam-buffer-toggle :wk "buffer")
...
(use-package org-modern
  :wal-ways nil
  :custom
  (org-modern-star nil) ; Handled by `org-bullets'.
...
(use-package org-sticky-header
  ...
  (org-sticky-header-prefix
   (concat (doom-modeline-icon 'fileicon "org" nil nil
...
`universal-argument')."
  (interactive)
  (if-let* ((cont (and (not (wal/univ-p))
                       (if wal/use-projectile
...
(wal/define-expansion-pack javascript
  :packages (js2-mode json-mode)
  :extras (rjsx-mode typescript-mode add-node-modules-path prettier))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
(wal/define-expansion-pack shell
  :packages (crontab-mode fish-mode))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
(wal/define-expansion-pack structured
  :packages (markdown-mode yaml-mode))
#+END_SRC
...
For these modes, `company-idle-delay' and `corfu-auto' will be
set to nil instead of 0.0."
  :type '(repeat symbol)
  :group 'wal-lsp)
...
(use-package lsp-mode
  ...
  (wal/major
    ...
    "l" '(lsp-avy-lens :wk "jump to lens")
    "x" '(lsp-execute-code-action :wk "execute action"))
  :delight
  (lsp-mode " lsp")
  (lsp-lens-mode " lns"))
#+END_SRC
...
(use-package consult-lsp
  :wal-ways nil
  :general
  (wal/major :keymaps 'lsp-mode-map
    "s" '(consult-lsp-file-symbols :wk "jump to symbol"))
...
#+BEGIN_SRC emacs-lisp
(defvar wal/setup-alist '()
  "Alists of already performed setups.")

(defun wal/setup-p (sym)
  "Check if SYM has already been setup.

If return nil after setting it to t."
  (if (car (assoc sym wal/setup-alist))
      t
    (message "Setting up %s" sym)
    (push `(,sym . t) wal/setup-alist)
    nil))
...
[fn:20] Mostly from MELPA. /Note/ that many packages bind keys. Check
