...
actions, mostly through to a =transient= bound to the same key (=.= by
default).

Leader key =captain= (=/= by default) gives access to minor-mode[fn:11]
command maps and transients. The package prefixes are:
...
(use-package kmacro
  ...
  (wal/captain "K" '(:keymap kmacro-keymap :wk "kmacro (map)"))
  :bind
  ("C-c k" . wal/kmacro)
  :ensure nil)
#+END_SRC
...
(use-package bookmark
  ...
  (:map bookmark-map
   ...
   ("L" . bookmark-load))
  :general
  (wal/captain "B" '(:keymap bookmark-map :wk "bookmark (map)"))
  :ensure nil)
#+END_SRC
