...
Modes that should be on no matter what.
...
(defvar wal/commonly-ignored-major-modes
  '(dired-mode helpful-mode magit-status-mode magit-diff-mode)
...
(defun wal/configure-result-buffer-display (buffer &optional side)
  "Configure `display-buffer' for result BUFFER shown on SIDE."
  (let ((condition (wal/display-buffer-condition buffer)))
    (add-to-list 'display-buffer-alist
                 `(,condition
...
(defun wal/configure-help-buffer-display (buffer)
  "Configure `display-buffer' for help BUFFER."
  (let ((condition (wal/display-buffer-condition buffer)))
    (add-to-list 'display-buffer-alist
                 `(,condition
                   (display-buffer-reuse-window display-buffer-in-side-window)
...
;;; Windows/Frames.
(global-set-key (kbd "H-q") 'kill-buffer-and-window) ; C-x 4 0
(global-set-key (kbd "H-5") 'make-frame-command) ; C-x 5 2

(wal/general "ws" '(wal/split-window-the-other-way :wk "split the other way"))

;; Enlarging/shrinking if we don't want `golden-ratio'.
(global-set-key (kbd "C-H-h") #'wal/shrink-window-horizontally)
(global-set-key (kbd "C-H-l") #'wal/enlarge-window-horizontally)
(global-set-key (kbd "C-H-k") #'wal/enlarge-window)
(global-set-key (kbd "C-H-j") #'wal/shrink-window)
...
(defvar wal/fixed-fonts
   '("mononoki" "Input Mono" "Fira Code" "Liberation Mono" "DejaVu Sans Mono")
...
at the end of this readme or the [[https://github.com/emacs-tw/awesome-emacs][awesome-emacs]] project.
...
(use-package avy
  ...
  :bind
  (("H-V" . wal/avy-goto-line-and-char)
   ("H-v" . avy-goto-line)))
#+END_SRC
...
(use-package crux
  ...
  (wal/general "ei" '(crux-find-user-init-file :wk "open init file")
               "br" '(crux-rename-buffer-and-file :wk "rename buffer and file")
               "gk" '(crux-kill-other-buffers :wk "kill other buffers")
...
This will mess up your windows!
...
(use-package debug
  ...
  :ensure nil
  :config
  (wal/configure-result-buffer-display 'debugger-mode 'bottom))
#+END_SRC
...
(use-package golden-ratio
  ...
  ;; This doesn't work for me, see alternative solution above.
...
(defvar wal/ivy-rich-transformer
  '(:columns
  ...
  "Transformer for switching buffers.")
...
(use-package ivy-rich
  ...
  (plist-put ivy-rich-display-transformers-list 'wal/switch-buffer wal/ivy-rich-transformer)
  (plist-put ivy-rich-display-transformers-list 'wal/ivy-switch-ignored-buffers wal/ivy-rich-transformer)
  (ivy-rich-mode 1))
#+END_SRC
...
(use-package lsp-mode
  ...
  (lsp-pyls-plugins-pylint-args (vector "--disable=import-error"))
  :config
...
Don't need this using =pyls= anymore, thankfully.
...
(use-package poetry
  ...
  :config
  (poetry-tracking-mode) ; Turn on global minor mode.
  (wal/configure-shell-buffer-display "\\*poetry-shell\\*")
  :general
  (wal/major :keymaps '(python-mode-map dired-mode-map)
...
I'm now a fan of =dired=, but sometimes the "ineluctable modality of the visible"
is nice, so let's show some /dirs/.
...
(defun wal/csharp-mode ()
  "Hooks for csharp mode."
  (message "Ouch!")
  (wal/disable/tabs)
  (lsp) ; Run `lsp-install-servver' to install.
  (flycheck-mode 1))
...
(use-package elixir-mode
  ...
  :general
  (wal/major "f" '(elixir-format :wk "format")))
#+END_SRC
...
(use-package org
  ...
  (org-tag-alist
      '(;; depth
        ...
        ("@edu" . ?e)))
  :bind
  (:map org-mode-map ; For source blocks.
   ("C-;" . evilnc-comment-or-uncomment-lines)))
