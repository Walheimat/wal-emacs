...
Leader key =major= (=.= by default) is responsible for actions that are
relevant to the current buffer, mostly through a =major-mode=-related
=transient= bound to the same key (=.= by default).
...
(cl-defmacro wal/major-mirror! (key fun mfun &rest args)
  "Bind FUN to KEY, MFUN in the sink.

All ARGS are passed to both definers."
  (declare (indent defun))
  `(progn
    (wal/major ,@args ,key ,fun)
    (wal/major-sink ,@args ,key ,mfun)))
...
(defvar wal/leader-with-sink '(wal/general wal/colonel wal/major wal/captain wal/lieutenant))
...
(use-package consult
  ...
  (wal/major-mirror! "c" 'consult-mode-command 'consult-minor-mode-menu)
  (general-define-key
...
(use-package projectile
  ...
  (wal/captain-sink "p" '(:keymap projectile-command-map :wk "projectile (map)"))
  (projectile-mode +1)
  :custom
...
"
    ...
    ("q" nil "quit"))
  (wal/major-sink :keymaps 'smerge-mode-map
...
(use-package flycheck
  ...
  :general
  (wal/major-sink :keymaps 'flycheck-mode-map
...
(use-package flyspell
  :config
  (advice-add #'flyspell-mode :override #'wal/flyspell)
  :custom
...
(use-package lsp-mode
  ...
  (wal/major-sink :keymaps 'lsp-mode-map
    "l" '(:keymap lsp-command-map :wk "lsp"))
  :custom
  (lsp-headerline-breadcrumb-enable nil)
  (lsp-keymap-prefix (wal/prefix-user-key (concat wal/major-key " " wal/major-key " " "l")))
...
(use-package verb
  ...
  (wal/major-sink :keymaps '(verb-mode-map verb-response-body-mode-map)
