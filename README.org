...
Comment and uncomment with less of a dance.
...
(defun wal/configure-consult ()
 ...
 (use-package consult
  ...
  (wal/major :keymaps 'outline-mode-map :major-modes t
    "g" '(consult-outline :wk "go to outline"))
  (wal/lieutenant
...
(defun wal/consult-persp-switch-to-buffer (buffer &optional norecord)
  "Switch to BUFFER.

Allow `consult--buffer-display' to be set to use
...
(use-package perspective
  ...
  (if (eq wal/completion-provider 'ivy)
    ...
    (setq consult--buffer-display #'wal/consult-persp-switch-to-buffer))
  (persp-mode)
  :custom
...
(defun wal/find-dart-flutter-sdk-dir ()
 "Find the Dart Flutter SDK directory."
  (when-let* ((sdk-dir (string-trim (shell-command-to-string "flutter sdk-path"))))
    sdk-dir))
...
[fn:6] Not including where =:tangle no= was set, 167 (on my last count)
