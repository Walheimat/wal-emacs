...
+ =C-c b <key>= toggles (=b=), shows (=s=) bookmarks inter alia.
+ =C-c f <key>= runs ag. _Requires ag_!
...
+ =C-c v <key>= uses =avy= to go (=g=) places inter alia.
...
(wal/general "c" '(wal/readme-config-open :wk "open config"))
...
(wal/lieutenant "t" '(wal/transparency :wk "adjust transparency"))
...
(wal/lieutenant "l" '(wal/light-switch :wk "theme light switch"))
...
(wal/lieutenant "f" '(wal/fonts--fixed-select :wk "select (fixed) font"))
...
(wal/general "s" '(wal/split-window-the-other-way :wk "split window again"))
#+END_SRC
...
(use-package avy
  :bind
  (("H-v"       . avy-goto-line)
   ("C-c v g l" . avy-goto-line)
   ("C-c v g c" . avy-goto-char)
   ("C-c v g w" . avy-goto-whitespace-end)
   ("C-c v c"   . avy-copy-line)
   ("C-c v k"   . avy-kill-whole-line)))
#+END_SRC
...
(use-package consult
  ...
  (wal/general "f" '(consult-focus-lines  :wk "focus-lines")
               "m" '(consult-mark :wk "go to mark"))
  (wal/major "c" '(consult-mode-command :wk "invoke mode command")
             :keymaps 'org-mode-map
             "h" '(consult-outline :wk "jump to heading"))
  (wal/lieutenant "g" '(consult-grep :wk "grep")))
...
(use-package consult-flycheck
  ...
  (wal/captain :keymaps 'flycheck-mode-map
               "f" '(consult-flycheck :wk "find error")))
#+END_SRC
...
(use-package counsel
  ...
  (wal/general "t" '(counsel-load-theme      :wk "load theme")
               ...
               "a" '(counsel-linux-app       :wk "launch app")
               "d" '(counsel-search          :wk "duck-duck-go"))
  :bind
  ("M-x"     . counsel-M-x)
  ("<menu>"  . counsel-M-x)
...
(use-package docker
  ...
  :bind
  (("H-k"     . docker)
   ("C-c k u" . docker-compose-up)
   ("C-c k e" . docker-compose-exec)
   ("C-c k l" . docker-compose-logs)))
#+END_SRC
...
(use-package eww
  :ensure nil
  :general
  (wal/lieutenant "w" '(eww :wk "www the web")))
#+END_SRC
...
(use-package flyspell
  ...
  :general
  (wal/colonel "S" '(flyspell-mode :wk "flyspell")
...
(use-package flyspell-correct
  ...
  :general
  (wal/captain :keymaps flyspell-mode-map
               "n" '(flyspell-correct-next     :wk "correct next error")
               "p" '(flyspell-correct-previous :wk "correct prev error")))
...
(use-package helpful
  ...
  :general
  (wal/major :keymaps 'emacs-lisp-mode-map
             "p" '(helpful-at-point          :wk "helpful at point")
...
(use-package ivy
  ...
  (ivy-wrap                t)
  :general
  (wal/captain "i" '(ivy-resume :wk "resume ivy"))
  :bind
...
(use-package lsp-mode
  ...
  (wal/captain :keymaps 'lsp-mode-map
               ...
               "o" '(lsp-ui-doc-focus-frame :wk "focus doc frame"))
  :custom
...
(use-package prettier
  ...
  (wal/major :keymaps 'js2-mode-map
             "p" '(prettier-prettify :wk "prettify")))
#+END_SRC
...
(use-package swiper
  ...
  :general
  (wal/lieutenant "b" '(swiper-all :wk "search all buffers"))
  :bind
  (("H-s" . swiper)
   :map swiper-map
   ("C-c v" . swiper-avy)))
...
(use-package yasnippet
  ...
  (wal/captain :keymaps 'yas-minor-mode-map
               "v" '(yas-visit-snippet-file :wk "visit snippet file")
               "c" '(yas-new-snippet        :wk "create new snippet"))
...
(use-package org-super-agenda
  ...
  (wal/major :keymaps 'org-agenda-keymap
             "w" '(org-agenda-write :wk "write agenda"))
  :bind
  ("H-a"   . org-agenda))
#+END_SRC
...
(use-package org-roam
  ...
  :general
  ;; theoretically these should be `wal/general' but these are minor of major 
  (wal/captain :keymaps 'org-mode-map
               "i" '(org-roam-insert           :wk "roam insert")
               "m" '(org-roam-insert-immediate :wk "roam insert immediate")
               "r" '(org-roam                  :wk "roam")
               "f" '(org-roam-find-file        :wk "roam find file")
               "g" '(org-roam-graph            :wk "roam graph"))
