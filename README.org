...
(wal/general "i" '(wal/open-init :wk "open init"))
...
(defhydra wal/text-scale (:timeout 2 :hint nil)
  "
_i_ncrease, _d_ecrease or _r_eset text size. Or _q_uit.
"
...
and preferred fonts are available, return the first of those instead."
  (let* ((available-fonts (--filter (find-font (font-spec :name it)) fonts))
         (preferred (and prefer (--filter (-contains? wal/preferred-fonts it)
                                          available-fonts))))
    (if preferred
        (car preferred)
      (car available-fonts))))
...
(use-package yasnippet
  :delight " yas"
  :hook ((yas-before-expand-snippet . wal/yas-before-expand-snippet)
...
(defun wal/configure-smartparens ()
  ...
  (defhydra wal/smartparens (:hint nil)
    "
  ^_p_^
_b_   _f_ Navigate pairs. Re_w_rap/_s_plice. Or _q_uit.
  ^_n_^
"
    ("b" sp-backward-sexp)
    ("f" sp-forward-sexp)
...
(use-package org-roam
  ...
  :commands (org-roam-buffer-display-dedicated
             org-roam-capture
...
(defun wal/dired-from-home (dirname &optional switches)
  "Open DIRNAME local to HOME directory using SWITCHES."
...
to set your `.dir-locals.el'."
  ...
  (ignore-errors
    (let* ((venv (poetry-venv-exist-p))
           (pylsp (file-exists-p (expand-file-name "bin/pylsp" venv)))
...
(use-package poetry
  :hook (python-mode . wal/poetry-lsp)
  :config
