...
quick-access editing or editing-adjacent actions.

Leader key =major= (=<.>= by default) prefixes major-mode-specific
actions.

Leader key =captain= (=</>= by default) prefixes minor-mode-specific
actions, as well as packages that have their own ecosystem like
=org-roam= and =magit=. The prefixes, sometimes depending on the
respective global minor-mode being on, are:
...
Leader key =lieutenant= prefixes non- or semi-mode package actions with
a editing emphasis. The package prefixes are:
      ...
      [["Actions"
       ...
       ["Go to"
        ("h" "heading" consult-org-heading
         :inapt-if-not (lambda () (eq wal/completion-provider 'vertico)))
        ("d" "packages" wal/dired-packages)]
       ["Customization"
        ("g" "group" wal/customize-group)]])
    (define-key wal/config-mode-map (kbd (wal/prefix-user-key "?")) #'wal/config-dispatch)))
#+END_SRC
