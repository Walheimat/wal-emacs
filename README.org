...
I try to have most actions use user-reserved =C-c <key>= combinations[fn:7],
but some other overriding mappings snuck in.

+ =C-c a <key>= opens today's (=t=), this week's (=w=) or my personal (=p=) agenda.
+ =C-c c= (un-)comments.
+ =C-c f <key>= runs ag (=f= for generic, =p= for in-project search). _Requires ag_!
...
The five =general= mappings perform useful but never
essential actions.
...
(use-package general
  ...
  (general-create-definer wal/general
    :prefix-command 'wal/general-prefix-command
    :prefix-name    "<general>"
    :prefix          wal/general-key)
  (general-create-definer wal/colonel
    :prefix-command 'wal/colonel-prefix-command
    :prefix-name    "<colonel>"
    :prefix         wal/colonel-key)
  (general-create-definer wal/major
    :prefix-command 'wal/major-prefix-command
    :prefix-name    "<major>"
    :prefix         wal/major-key)
  (general-create-definer wal/captain
    :prefix-command 'wal/captain-prefix-command
    :prefix-name    "<captain>"
    :prefix         wal/captain-key)
  (general-create-definer wal/lieutenant
    :prefix-command 'wal/lieutenant-prefix-command
    :prefix-name    "<lieutenant>"
...
(wal/colonel "E" '(wal/emergency-mode :wk "toggle emergency"))
(wal/colonel "N" '(linum-mode         :wk "toggle line numbers"))
#+END_SRC

*** About Leader Keys

Leader key =general= mainly prefix actions that should be possible
no matter the major mode.

Leader key =colonel= mainly prefix actions that make sense within
the current major mode, like toggling minor modes.

Leader keys =major=, =captain= and =lieutenant= all bind
additional actions that didn't fit elsewhere.
The letters follow the custom binding scheme,
so =f= will perform an =ag= action.
...
(global-set-key (kbd "H-c") 'wal/readme-config-open)
...
(use-package ace-window
  ...
  :general
  (wal/general "s" '(wal/split-window-the-other-way :wk "split the other way"))
  :bind (("H-w"   . ace-window)
         ("C-c w" . ace-window)))
#+END_SRC
...
(use-package ag
  ...
  (ag-ignore-list      '(".git"
                         ...
                         "deps"))
  :general
  (wal/major   "f" '(ag-files        :wk "find in files of type"))
  (wal/captain "f" '(ag-kill-buffers :wk "kill ag buffers"))
  :bind (("C-c f f" . ag)
         ("C-c f p" . ag-project)
         ("H-f"     . ag-project)
...
(use-package avy
  :general
  (wal/major      "v" '(avy-goto-whitespace-end :wk "go to whitespace"))
  (wal/captain    "v" '(avy-kill-whole-line     :wk "kill specific line"))
  (wal/lieutenant "v" '(avy-copy-line           :wk "copy line and paste above"))
...
(use-package consult
  ...
  (wal/general "g" '(consult-grep         :wk "grep"))
  (wal/general "m" '(consult-mark         :wk "go to mark"))
  (wal/colonel "c" '(consult-mode-command :wk "invoke mode command"))
  (wal/colonel org-mode-map "h" '(consult-outline :wk "jump to heading")))

(use-package consult-flycheck
  :init
  (advice-add 'consult-flycheck :around #'wal/advise-selectrum)
  :general
  (wal/colonel flycheck-mode-map "f" '(consult-flycheck :wk "find error")))
#+END_SRC
...
(use-package counsel
  :after ivy
  :general
  (wal/general "t" '(counsel-load-theme      :wk "load theme"))
  (wal/general "h" '(counsel-command-history :wk "command history"))
  (wal/general "v" '(counsel-set-variable    :wk "set variable"))
  (wal/general "r" '(counsel-linux-app       :wk "launch app"))
...
(use-package crux
  :general
  (wal/general "." '(crux-find-user-init-file :wk "find init file"))
...
(use-package dap-mode
  ...
  (require 'dap-python)
  :general
  (wal/colonel prog-mode-map "D" '(dap-mode  :wk "dap mode"))
  (wal/colonel dap-mode-map  "d" '(dap-hydra :wk "debug")))
#+END_SRC
...
(use-package dired
  ...
  (("H-d" . dired-jump)
   :map dired-mode-map
   ("," . dired-display-file)   ;; overrides dired-do-run-mail
   ("-" . dired-up-directory))) ;; overrides negative-argumen
#+END_SRC
...
(use-package docker
  ...
  :general
  (wal/major      "k" '(docker-compose-up   :wk "docker-compose up"))
  (wal/captain    "k" '(docker-compose-exec :wk "docker-compose exec"))
  (wal/lieutenant "k" '(docker-compose-logs :wk "docker-compose logs"))
  :bind (("C-c k" . docker)
         ("H-k"   . docker)))
#+END_SRC
...
(use-package flycheck
  ...
  :general
  (wal/colonel prog-mode-map "Q" '(flycheck-mode :wk "toggle flycheck"))
  (wal/colonel flycheck-mode-map "e" '(flycheck-next-error :wk "next error"))
...
(use-package flyspell
  ...
  :general
  (wal/colonel "S" '(flyspell-mode :wk "toggle spell-check"))
  (wal/colonel prog-mode-map "S" '(wal/flyspell-prog-mode :wk "toggle spell-check (prog)"))
  (wal/colonel flyspell-mode-map "s" '(flyspell-buffer :wk "check spelling"))
  :custom
...
(use-package flyspell-correct
  :after flyspell
  :general
  (wal/colonel flyspell-mode-map "n" '(flyspell-correct-next     :wk "correct next error"))
  (wal/colonel flyspell-mode-map "p" '(flyspell-correct-previous :wk "correct prev error")))
...
(use-package git-timemachine
  ...
  :general
  (wal/colonel prog-mode-map "G" '(git-timemachine-toggle :wk "toggle timemachine")))
#+END_SRC
...
(use-package google-this
  ...
  :diminish
  :general
  (wal/major      "g" '(google-this        :wk "google this"))
  (wal/captain    "g" '(google-this-region :wk "google selected region"))
  (wal/lieutenant "g" '(counsel-search     :wk "search with counsel")))
#+END_SRC
...
(use-package helpful
  ...
  :general
  (wal/major      emacs-lisp-mode-map "h" '(helpful-at-point          :wk "helpful at point"))
  (wal/captain    emacs-lisp-mode-map "h" '(counsel-describe-variable :wk "describe variable"))
  (wal/lieutenant emacs-lisp-mode-map "h" '(counsel-describe-function :wk "describe function"))
  :bind
...
(use-package highlight-indent-guides
  ...
  :general
  (wal/colonel prog-mode-map "I" '(highlight-indent-guides-mode :wk "toggle indent highlighting"))
  (wal/colonel yaml-mode-map "I" '(highlight-indent-guides-mode :wk "toggle indent highlighting")))
#+END_SRC
...
(use-package highlight-thing
  ...
  :general
  (wal/colonel "H" '(highlight-thing-mode :wk "Highlight thing")))
#+END_SRC
...
(use-package ivy
  ...
  (ivy-wrap                t)
  :general
  (wal/general "i" '(ivy-resume :wk "resume ivy"))
...
(use-package lsp-mode
  :delight " lsp"
  :general
  (wal/colonel prog-mode-map   "L" '(lsp-mode :wk "toggle LSP"))
  (wal/colonel lsp-ui-mode-map "o" '(lsp-ui-doc-focus-frame :wk "focus doc frame"))
  :custom
...
(use-package perspective
  ...
  :general
  (wal/major      "i" '(persp-kill          :wk "change perspective name"))
  (wal/captain    "i" '(persp-add-buffer    :wk "add buffer to perspective"))
  (wal/lieutenant "i" '(persp-remove-buffer :wk "remove buffer from perspective"))
...
(use-package prettier
  ...
  :general
  (wal/colonel js2-mode-map "p" '(prettier-prettify :wk "prettify")))
#+END_SRC
...
(use-package projectile
  :delight " pjt"
  :general
  (wal/major      "p" '(projectile-find-file    :wk "find file in project"))
  (wal/captain    "p" '(projectile-find-dir     :wk "find dir in project"))
  (wal/lieutenant "p" '(projectile-kill-buffers :wk "kill project buffers"))
...
(use-package sudo-edit
  ...
  :general
  (wal/general "e" '(sudo-edit-find-file :wk "find file as root")))
#+END_SRC
...
(use-package swiper
  ...
  :general
  (wal/major      "s" '(swiper-all            :wk "search all buffers"))
  (wal/captain    "s" '(swiper-multi          :wk "search multiple buffers"))
  (wal/lieutenant "s" '(swiper-thing-at-point :wk "search thing at point"))
  :bind (("C-c s" . swiper)
         ("H-s"   . swiper)
         :map swiper-map
         ("C-c v" . swiper-avy)))
...
(use-package typo
  ...
  :delight " typ"
  :general
  (wal/colonel text-mode-map "T" '(typo-mode :wk "toggle typo mode")))
#+END_SRC
...
(use-package whitespace-mode
  :ensure nil
  :hook (whitespace-mode . (lambda() (highlight-indent-guides-mode -1)))
  :general
  (wal/colonel "W" '(whitespace-mode :wk "toggle whitespaces")))
#+END_SRC
...
(use-package yasnippet
  ...
  :delight " yas"
  :general
  (wal/colonel    "Y" '(yas-minor-mode         :wk "toggle snippet mode"))
  (wal/major      "y" '(yas-visit-snippet-file :wk "visit snippet file"))
  (wal/captain    "y" '(yas-new-snippet        :wk "create new snippet"))
...
(use-package org-super-agenda
  ...
  :general
  (wal/colonel org-agenda-keymap "w" '(org-agenda-write :wk "write agenda"))
...
(use-package org-tree-slide
  ...
  :general
  (wal/colonel org-mode-map "p" '(org-tree-slide-mode :wk "presentation"))
...
(use-package python-mode
  :ensure nil
  :general
  (wal/colonel python-mode-map "s" '(run-python               :wk "run pyshell"))
  (wal/colonel python-mode-map "r" '(python-shell-send-region :wk "send region"))
  (wal/colonel python-mode-map "b" '(python-shell-send-buffer :wk "send buffer"))
  (wal/colonel python-mode-map "f" '(python-shell-send-file   :wk "send file"))
...
[fn:5] 141 code blocks, to be exact.
