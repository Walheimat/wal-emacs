...
+ =H-[= toggles pop-ups using =popper= (or runs a =hydra= if focusing a pop-up)
+ =H-\= toggles =vterm=
+ =H-u= switches buffer using =consult= (or =counsel=)
...
(use-package verb
  ...
  (wal/major :keymaps 'org-mode-map
    "r" '(:ignore t :wk "verb")
    "rr" '(verb-send-request-on-point-other-window-stay :wk "make request")
    "rk" '(verb-kill-all-response-buffers :wk "kill response buffers")
    "rv" '(verb-set-var :wk "set var"))
