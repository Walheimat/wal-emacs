...
(defun wal/hook--treesit-ready-p (mode)
...
(defun wal/hook--safe-body (body)
  "Collect everything from BODY that's a key."
  (cl-loop for (key val)
           on body by 'cddr
           unless (memq key wal/hook--keywords)
           collect key
           and collect val))

(defun wal/hook--treesit (treesit name)
...
Retunrs cons cell of (TS-NAME . TS-READY)."
  (let* ((ts-seg (pcase treesit
         ...
         (ts-ready (and treesit (wal/hook--treesit-ready-p name))))

    (cons ts-name ts-ready)))

(defun wal/hook--get-function-name (name suffix &optional prefix)
...
The rest of the BODY will be spliced into the hook function."
  ...
  (let ((ts (wal/hook--treesit treesit name)))

    `(progn
       (defun ,(wal/hook--get-function-name name "hook" t) ()
         ,docs
         ,@(delete
            nil
            `(,(when messages `(wal/message-in-a-bottle ',messages))
              ,(when (not shallow) `(wal/hook--tabs ,tabs))

              ,@(wal/hook--safe-body body)
              ...
              ,(when captain
                 `(local-set-key
                   ...
                   ',(wal/hook--get-function-name name "dispatch" t))))))

       (add-hook
        ',(if (cdr-safe ts)
              (wal/hook--get-function-name (intern (car-safe ts)) "hook")
            (wal/hook--get-function-name name "hook"))
        ',(wal/hook--get-function-name name "hook" t)))))

(cl-defmacro wal/hook--ligatures (name &key ligatures treesit &allow-other-keys)
  "Set up LIGATURES respecting TREESIT for NAME."
  (declare (indent defun))

  (when-let ((non-empty ligatures)
             (ts (wal/hook--treesit treesit name)))

    (if (cdr-safe ts)
        `(wal/set-ligatures '(,name ,(intern (car-safe ts))) ',ligatures)
      `(wal/set-ligatures ',name ',ligatures))))

(cl-defmacro wal/hook--lsp (&key lsp lsp-ignores &allow-other-keys)
  "Set up LSP and LSP-IGNORES."
  (when (and lsp lsp-ignores)
    `(with-eval-after-load 'lsp-mode
       (wal/append 'lsp-file-watch-ignored-directories ',lsp-ignores))))

(cl-defmacro wal/hook (name docs &rest args)
...
The rest of the ARGS will be spliced into the hook function."
  (declare (indent defun) (doc-string 2))
  `(progn
     (wal/hook--function ,name ,docs ,@args)

     (wal/hook--ligatures ,name ,@args)
