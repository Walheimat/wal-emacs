...
(defvar wal/language-packs nil
  "Packs of language packages to be installed using `wal/install-language-pack'.

Individual languages build this list using macro `wal/define-language-pack'.")

(cl-defmacro wal/define-language-pack (lang &key packages)
  "Define a language pack of PACKAGES for LANG."
  `(add-to-list 'wal/language-packs '(,lang . ,packages)))
...
(wal/define-language-pack elixir
  :packages
  (elixir-mode))
#+END_SRC
...
(wal/define-language-pack python
  :packages
  (python-isort
   pyvenv
   poetry))
...
(wal/define-language-pack javacsript
  :packages
  (js2-mode
   rjsx-mode
   typescript-mode
...
(wal/define-language-pack shell
  :packages
  (crontab-mode
   fish-mode))
#+END_SRC
...
(wal/define-language-pack structured
  :packages
  (markdown-mode
   yaml-mode))
#+END_SRC
...
(wal/define-language-pack flutter
  :packages
  (dart-mode
   lsp-dart))
#+END_SRC
