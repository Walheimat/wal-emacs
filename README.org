...
(defvar wal/fixed-fonts
   '("mononoki" "Fira Code" "Liberation Mono" "DejaVu Sans Mono")
...
(use-package ag
  :config
  (wal/configure-result-buffer-display "\\*ag-search\\*")
...
Only goes to line if line is empty."
  (interactive)
  (avy-goto-line)
...
#+BEGIN_SRC emacs-lisp
(defun wal/aw-switch-maybe-golden-ratio ()
