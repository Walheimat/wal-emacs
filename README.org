...
#+BEGIN_SRC emacs-lisp
(defvar wal/org-fixed-pitch-faces
  '(org-code org-block org-verbatim org-table)
  "Faces that should inherit from `shadow' and `fixed-pitch'.")
...
(defun wal/configure-org ()
  ...
  ;; Make sure that `variable-pitch-mode' does not extend to certain faces.
  (--each wal/org-fixed-pitch-faces
    (set-face-attribute it  nil :inherit '(shadow fixed-pitch))))
