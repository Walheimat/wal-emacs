...
These hydras are mapped to:

+ =C-M-v= to scroll other window
+ =C-M-x= to scale text
+ =C-M-z= to resize window.
...
(general-define-key
 "C-M-x" '(wal/text-scale/body :wk "text scale")
 "C-M-z" '(wal/resize-window/body :wk "resize window")
 [remap scroll-other-window] '(wal/scroll-other-window/body :wk "scroll other window"))
...
Mouse yanks at point, no double spaces in sentences.
...
"
    ("c" popper-cycle)
    ("o" wal/popper-open-all :exit t)
    ("b" wal/popper-bury-all :exit t)
    ("q" nil "quit")))
