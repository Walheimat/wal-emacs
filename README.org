...
Bind some useful functions and up some values.
...
Allows using variable and fixed fonts in the same buffer.
...
Allows padding and centering text.
...
(defun wal/lsp (&optional disable)
  "Activate LSP client.
If called with `universal-argument', DISABLE.
This also (re-)sets values for `company'."
  (interactive "P")
  (pcase wal/lsp-client
    ('lsp-mode (if disable
                   (progn
                     ...
                     (lsp-mode -1))
                 (lsp-deferred)))
    ;; FIXME: Still need to find a way to properly
    ;;        disable `eglot' so it can also be re-enabled.
    ('eglot (if disable
  ...
  (if disable
      (progn
        (wal/reset-to-standard 'company-idle-dealy t)
        (wal/reset-to-standard 'company-backends t))
