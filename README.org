...
=H-l= activates =ryo-modal-mode= for quick movements.
...
(use-package ryo-modal
  ...
  :config
  (wal/when-ready
   (setq ryo-modal-cursor-color (face-attribute 'highlight :background)))
...
(defvar wal/avy-ryo-buffer nil)

(defun wal/avy-ryo-before ()
  "Store the current buffer in a variable."
  (setq wal/avy-ryo-buffer (current-buffer)))

(defun wal/avy-ryo-after ()
  "Exit `ryo-mode' in stored buffer if we moved to a different buffer."
  (when wal/avy-ryo-buffer
    (unless (equal wal/avy-ryo-buffer (current-buffer))
      (with-current-buffer wal/avy-ryo-buffer
        (ryo-modal-mode -1))
      (setq wal/avy-ryo-buffer nil))))
...
(use-package avy
  ...
  :ryo
  ("s" avy-goto-line-above)
  ("f" avy-goto-line-below)
  ("e" avy-goto-word-0-above)
  ("d" avy-goto-word-0-below)
  ("a" wal/avy-goto-word)
  (:first '(wal/avy-ryo-before) :then '(wal/avy-ryo-after))
  ("w" avy-goto-line)
  ("r" avy-goto-char))
#+END_SRC
...
(use-package consult
 ...
 :ryo
 ("t" consult-org-heading :mode 'org-mode))
#+END_SRC
