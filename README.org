...
(cl-defmacro wal/hook
    (name
     docs
     &body body
     &key messages lsp lsp-ignores tabs prog-like captain corfu shallow functions
     &allow-other-keys)
...
The rest of the BODY will be spliced into the lambda."
  ...
  (let ((target (intern (concat (symbol-name name) "-hook")))
        (safe-body (cl-loop for (key val) on body by 'cddr
                            unless (memq key '(:messages :lsp :lsp-ignores :tabs :prog-like :captain :corfu :shallow :functions))
                            collect key
                            and collect val))
