...
Leader key =colonel= (=<;>= by default) provides an additional layer for
the default user prefix (right hand) and additional actions (left
hand).
...
(use-package popper
  ...
  :general
  (wal/colonel :keymaps 'popper-mode-map
