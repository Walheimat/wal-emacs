...
+ =H-n= searches with =ag= in project
...
(defun wal/configure-ace-window ()
  ...
  (cond
   ((eq wal/completion-provider 'vertico)
    (advice-add 'aw--switch-buffer :override #'wal/advise-aw--switch-buffer-consult))
   ((eq wal/completion-provider 'ivy)
    (advice-add 'aw--switch-buffer :override #'wal/advise-aw--switch-buffer-ivy))
   (t nil)))
...
(use-package consult
  ...
  (wal/lieutenant
    ...
    "ug" '(consult-grep :wk "grep")
    "uf" '(consult-focus-lines :wk "focus lines")
    "ul" '(consult-line :wk "search lines")
    "um" '(consult-kmacro :wk "macro")
    "ur" '(consult-recent-file :wk "recent")
    "ux" '(consult-file-externally :wk "open externally"))
  :after vertico)
...
#+BEGIN_SRC emacs-lisp
(defcustom wal/use-perspectives nil
...
(use-package dired
  ...
  (("C-x x d" . wal/dired-from-home)
   ("C-x j" . dired-jump)
   ("C-x x j" . dired-jump-other-window)
   :map dired-mode-map
...
[fn:6] Not including where =:tangle no= was set, 166 (on my last count)
