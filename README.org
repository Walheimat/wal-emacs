...
(cl-defmacro wal/hook (name docs &body body &key messages lsp lsp-ignores tabs prog-like &allow-other-keys)
...
The rest of the BODY will be spliced into the lambda."
  ...
  (let ((target (intern (concat (symbol-name name) "-hook")))
        (safe-body (cl-loop for (key val) on body by 'cddr
                            unless (memq key '(:messages :lsp :lsp-ignores :tabs :prog-like))
                            collect key
                            and collect val))
...
(use-package mhtml-mode
  ...
  (wal/hook mhtml-mode
    ...
    :lsp t
    :messages '("Hodge-podge mark-p language"))
...
(defun wal/check-coverage ()
  ...
  (interactive)
  (let* ((suffix "coverage")
         (command "cask exec ert-runner && cat coverage.txt"))
...
(defun wal/create-json-coverage ()
  ...
  (interactive)
  (let* ((suffix "json-coverage")
         (command "export COVERAGE_WITH_JSON=true && cask exec ert-runner"))
...
(defun wal/coverage--execute (command on-success on-failure buffer-suffix)
...
name are deleted beforehand."
  ...
  (let* ((dir wal/emacs-config-package-path)
         ...
         (buffer-name (format "-%s\\*" buffer-suffix)))
    ...
    (wal/async-process full-command on-success on-failure buffer-suffix)))
#+END_SRC
