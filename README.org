...
Get even more juice out of it with custom keywords.
...
(when (wal/capture-flag 'doctor)
  (setq wal/flag-doctor t))

;; Flag `--mini' will only load packages that are part of the minimal
;; configuration.
(when (wal/capture-flag 'mini)
  (setq wal/flag-mini t))

;; Flag `--deny' will prevent `use-package' from ensuring packages.
(when (wal/capture-flag 'deny)
  (setq wal/flag-deny t))
#+END_SRC
...
to `lsp-file-watch-ignored-directories'.
...
(use-package dashboard
  ...
  (advice-add 'dashboard-random-footer :override
...
(wal/define-expansion-pack python
  ...
  :packages (pyvenv python-isort python-pytest python-black)
...
TOML."
  (when-let* ((marker (or marker-file  "pyproject.toml"))
              ...
              (config-file (concat project-directory marker)))
    t))
...
#+BEGIN_SRC emacs-lisp
(defun wal/lsp-pyright-set-venv ()
  "Set the current virtual environment."
  (defvar pyvenv-virtual-env)
  (setq-local lsp-pyright-venv-path pyvenv-virtual-env))
...
Versatile but increasingly slow language server that supports =jedi=,
=black=, =mypy=, =flake8=, =pycodestyle=, =pylint= ... the list goes on.

#+BEGIN_SRC emacs-lisp
(defun wal/lsp-pylsp-set-venv ()
  "Set the current virtual environment."
  (defvar pyvenv-virtual-env)
  (setq-local lsp-pylsp-plugins-jedi-environment pyvenv-virtual-env))

;; This package is included in `lsp-mode'.
(use-package lsp-pylsp
  :init
  (add-hook
   'pyvenv-post-activate-hooks
   #'wal/lsp-pylsp-set-venv)
  :ensure nil)
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
(defun wal/poetry-activate-venv ()
  "Activate venv using poetry.

This also works if the project isn't a poetry project."
  (interactive)
  (condition-case err
      (when-let* ((venv (poetry-venv-exist-p))
                  (expanded (expand-file-name venv)))
        (setq-local python-shell-virtualenv-root expanded)
        (poetry-venv-workon))
    (error
...
(use-package poetry
  :wal-ways nil
  :hook (python-mode . wal/poetry-activate-venv)
...
#+BEGIN_SRC emacs-lisp
(defun wal/apply-padding-to-string (string)
...
INFO contains positioning information."
  (let* ((window-left (plist-get info :parent-window-left))
         ...
         ;; Offset the frame, taking both the right fringe as well as
         ;; the pixel-height of a lien into account.
         (p-window (plist-get info :parent-window))
         (p-line-height (with-selected-window p-window (line-pixel-height)))
         (parent-fringes (window-fringes p-window))
         (right-fringe (nth 1 parent-fringes ))
         (offset-x (+ p-line-height right-fringe))
...
[fn:20] Binding reference [[http://oremacs.com/lispy/][here]].

[fn:21] All languages listed [[https://emacs-lsp.github.io/lsp-mode/page/languages/][here]].
