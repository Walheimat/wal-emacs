...
#+BEGIN_SRC emacs-lisp
;; FIXME: These tests don't really work but succeed.

(ert-deftest test-wal/async-process ()
  (wal/async-process
   "echo \"the world is a vampire\""
   (lambda ()
     (with-current-buffer (get-buffer-create "*wal-async*")
       (should (string-match "the world is a vampire" (buffer-string)))))
   (lambda (_) nil)))

(ert-deftest test-wal/async-process--on-lisp-error ()
  (wal/async-process
   "echo \"the world is a vampire\""
   (lambda ()
     (error "Oops!"))
   (lambda (m)
     (should (string-match "Oops!" m)))))

(ert-deftest test-wal/async-process--on-command-error ()
  (wal/async-process
   "exit 1"
   (lambda ()
     (message "Shouldn't happen"))
   (lambda (m)
     (should (string-match "Should!" m)))))
#+END_SRC
