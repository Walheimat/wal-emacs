...
#+BEGIN_SRC emacs-lisp
(cl-defmacro parallel (a b)
  "Define a function composing A and B.

By default, A is called. B will be called if the default
`universal-argument' is present. A will be called with
`universal-argument' if the function is called with two `C-u' or
more.
...
Both functions will be called interactively."
  ...
  (let ((a-name (symbol-name a))
    ...
    `(defun ,(intern (concat a-name "//" b-name)) (&optional call-other)
...
(cl-defun junk-install-packages (packages &key delete-windows)
...
(defun junk-install-recipes (recipes)
  ...
  (let* ((sym (wal/maybe-intern pack))
    ...
    (cl-destructuring-bind
      ...
      (if (not item)
        ...
        (let* ((installed-packages (junk-install-packages packages :delete-windows t))
               (installed-recipes (junk-install-recipes recipes)))
...
(defvar wal/treesit-mode-lang-alist '((js-mode . javascript))
  "Alist mapping major modes to languages.")

(defun harpoon--treesit-ready-p (mode)
  "Check if treesit is available for MODE.

MODE is only checked if it has a language mapping."
  (and (wal/modern-emacs-p 29)
       (require 'treesit nil t)
       (treesit-available-p)
       (if-let* ((lang (cdr-safe (assoc mode wal/treesit-mode-lang-alist))))
           (treesit-ready-p lang t)
         t)))

(defmacro harpoon--tabs (tabs)
  "Set messages and indentation using TABS."
  `(progn
...
(defun harpoon--treesit (treesit name)
  "Get `treesit' values from using TREESIT and NAME.

Retunrs cons cell of (TS-NAME . TS-READY)."
  (let* ((ts-seg (pcase treesit
                   ('base "base")
                   (_ "ts")))

         (ts-name (thread-first
                    name
                    (symbol-name)
                    (split-string "-")
                    (cl-subseq 0 -1)
         ...
         (ts-ready (and treesit (harpoon--treesit-ready-p name))))

    (cons ts-name ts-ready)))

(defun harpoon--get-function-name (name suffix &optional prefix)
  "Get function name for NAME with SUFFIX.

If PREFIX is non-nil, prepend `wal/' to the function name."
  (let ((end (concat (symbol-name name) "-" suffix)))

    (if prefix
        (intern (concat "wal/" end))
      (intern end))))
...
(cl-defmacro harpoon--function
    (name
     ...
     treesit
     &allow-other-keys)
  "Create hook function NAME with DOCS.
...
If TREESIT is non-nil, set the hook for the base-mode.

The rest of the BODY will be spliced into the hook function."
  (declare (indent defun))

  (let ((ts (harpoon--treesit treesit name)))

    `(progn
       (defun ,(harpoon--get-function-name name "hook" t) ()
         ,docs
         ,@(delete
            nil
            `(,(when messages `(wal/message-in-a-bottle ',messages))
              ,(when (not shallow) `(harpoon--tabs ,tabs))
              ...
              ,(when captain
                 `(local-set-key
                   ...
                   ',(harpoon--get-function-name name "dispatch" t))))))

       (add-hook
        ',(if (cdr-safe ts)
              (harpoon--get-function-name (intern (car-safe ts)) "hook")
            (harpoon--get-function-name name "hook"))
        ',(harpoon--get-function-name name "hook" t)))))

(cl-defmacro harpoon--ligatures (name &key ligatures treesit &allow-other-keys)
  "Set up LIGATURES respecting TREESIT for NAME."
  (declare (indent defun))

  (when-let ((non-empty ligatures)
             (ts (harpoon--treesit treesit name)))

    (if (cdr-safe ts)
        `(wal/set-ligatures '(,name ,(intern (car-safe ts))) ',ligatures)
      `(wal/set-ligatures ',name ',ligatures))))

(cl-defmacro harpoon--lsp (&key lsp lsp-ignores &allow-other-keys)
  "Set up LSP and LSP-IGNORES."
  (when (and lsp lsp-ignores)
    `(with-eval-after-load 'lsp-mode
       (wal/append 'lsp-file-watch-ignored-directories ',lsp-ignores))))

(cl-defmacro harpoon (name docs &rest args)
...
The rest of the ARGS will be spliced into the hook function."
  (declare (indent defun) (doc-string 2))
  `(progn
     (harpoon--function ,name ,docs ,@args)
...
#+BEGIN_SRC emacs-lisp
(cl-defmacro wal/treesit (language &key replacement icons-name)
  "Remap LANGUAGE major-mode to tree-sitter variant if possible.

If REPLACEMENT is non-nil, use it instead of constructing
LANGUAGE-ts-mode for the variant.

If ICONS-NAME is non-nil, use it instead of LANGUAGE-mode to try
to set an icon."
  (declare (indent defun))

  (if-let* ((available (and (featurep 'treesit) (treesit-available-p)))
            (ready (treesit-ready-p language t))

            (sym-name (symbol-name language))
            (mode-name (intern (concat sym-name "-mode")))

            (ts-name (or replacement (intern (concat sym-name "-ts-mode")))))

      `(progn
         (message "Remapping %s to %s" ',mode-name ',ts-name)
         (add-to-list 'major-mode-remap-alist ',(cons mode-name ts-name))

         (with-eval-after-load 'all-the-icons
           (when-let ((setting (cdr (assoc ',(or icons-name mode-name) all-the-icons-mode-icon-alist)))
                      (name ',ts-name))

             (add-to-list 'all-the-icons-mode-icon-alist
                          (cons name setting)))))
    `(message "Not remapping %s" ',language)))
#+END_SRC
...
(wal/on-boot external
  ...
  (condition-case nil
      (junk-install-packages wal/core-packages :delete-windows t)
    (error
...
(use-package winner
  ...
  (wal/transient-define-prefix-once wal/winner ()
    "Quickly undo and redo window configurations."
  ...
  :general
  (wal/general "o" '(wal/winner :wk "winner")))
#+END_SRC
...
(use-package consult
  ...
  :wal-bind
  (("j" . consult-buffer)
   ("i" . consult-register-load//consult-register-store))
...
If you want a fully-fledged terminal emulator[fn:14].
...
#+BEGIN_SRC emacs-lisp
(use-package magit
  :commands (wal/magit-dispatch magit-status//wal/magit-dispatch)

  :init
  (parallel magit-status wal/magit-dispatch)
  ...
  :wal-bind
  ("m" . magit-status//wal/magit-dispatch)
...
(use-package smerge-mode
  ...
  :config
  (wal/transient-define-prefix-once wal/smerge ()
    "Resolve conflicts until you don't."
    [["Move"
...
(use-package js
  ...
  (wal/treesit javascript
    :replacement js-ts-mode
    :icons-name js-mode)
...
(use-package dap-mode
  ...
  (wal/transient-define-prefix-once wal/dap ()
    "Debug, transiently."
    [["Step"
     ...
     ["Debugging"
      ("s" "start" dap-debug)
      ("x" "disconnect" dap-disconnect)]
     ["Breakpoints"
      ("b" "toggle" dap-breakpoint-toggle)
      ("l" "list" dap-ui-breakpoints-list)]])
  ...
  :general
  (wal/major :keymaps 'lsp-mode-map "b" '(wal/dap :wk "debug"))
