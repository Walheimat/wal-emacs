...
+ =H-7= opens (and =H-8= cycles through) =popper= pop-up(s)
+ =H--= duplicates current line/region with =crux=
...
+ =H-n= searches with =ripgrep= (a =transient=) or =ag= (a =hydra=)
...
(use-package simple
  ...
  (wal/colonel-sink "f" '(auto-fill-mode :wk "auto-fill"))
...
(use-package find-func
  :general
  (wal/general "l" '(find-library :wk "find library"))
  :ensure nil)
#+END_SRC
...
(use-package crux
  ...
  :wal-bind
  (("9" . wal/crux-duplicate-current-line-or-region)
   ("0" . wal/crux-duplicate-and-comment-current-line-or-region)
...
(use-package dashboard
  ...
  :general
  (wal/general-sink "d"
    '(wal/show-dashboard :wk "show dashboard"))
...
You need to install the icons yourself[fn:4]. Packages =dired=
configures its own sub-package.
...
(use-package popper
  ...
  :wal-bind
  (:map popper-mode-map
   ("7" . popper-toggle-latest)
   ("8" . popper-cycle))
  :general
  (wal/captain :keymaps 'popper-mode-map
    "p" '(:ignore t :wk "popper")
    "pr" '(popper-toggle-type :wk "raise or drop")
    "pk" '(popper-kill-latest-popup :wk "kill latest")
    "pa" '(wal/popper-open-all :wk "open all"))
  (general-define-key
...
Highlight search results using the *Silver Searcher* or =ripgrep=.

Both require installation of their respective binary. You can get the
=ag= binary from [[https://github.com/ggreer/the_silver_searcher#installation][here]][fn:14], the =ripgrep= one from [[https://github.com/BurntSushi/ripgrep][here]].
...
(defun wal/configure-consult ()
 ...
 (use-package consult
  ...
  (("j" . consult-buffer))
  :general
  (wal/general "t" '(consult-theme :wk "theme"))
...
(declare-function persp-buffer-in-other-p "perspective.el")
...
(defun wal/consult-persp-switch (buffer-or-name)
  "Switch perspectives before switching to BUFFER-OR-NAME.

This function is a copy of `persp-switch-to-buffer' but always
checks if the buffer belongs to another perspective since
`consult' will have already switched in preview."
...
(defvar wal/consult-source-perspective
...
(defun wal/consult-buffer-query-outside-persp ()
  "Query for buffers outside of the current perspective.

This is used for the default buffer source when using perspectives."
  (let ((persp-bufs (persp-current-buffer-names)))
    (seq-filter (lambda (it) (not (memq it persp-bufs)))
...
completion provider also switches perspectives."
  ...
  (if (eq wal/completion-provider 'ivy)
    ...
    (with-eval-after-load 'consult
      (add-to-list 'consult-buffer-sources 'wal/consult-source-perspective)
      (consult-customize
       consult--source-buffer
       :default nil
       :name "Other Buffer"
       :action #'wal/consult-persp-switch
       :items #'wal/consult-buffer-query-outside-persp
       :preview-key nil))) ; Otherwise previews add buffers to perspective!
  (persp-mode))
...
Group directories first in =dired=, override some keybindings.
...
This depends on `wal/lsp-provider'."
  (interactive)
