...
(with-no-warnings
  ...
  (global-set-key (kbd (wal/key-combo-for-leader 'wal/whaler)) #'whaler)

  (wal/administrator "n" 'display-line-numbers-mode)

  (global-set-key (kbd (wal/key-combo-for-leader 'wal/major)) #'wal/major?)

  (when (wal/modern-emacs-p 29)
    (wal/editor "d" 'duplicate-dwim))

  (wal/editor "M-w" 'wal/kill-ring-save-whole-buffer)
...
(defcustom wal/leaders '((";" . wal/editor)
                         ("." . wal/major)
                         ("," . wal/consult)
                         ("/" . wal/ambassador)
                         ("'" . wal/roamer)
...
#+BEGIN_SRC emacs-lisp
(defvar wal/general-leaders '(wal/editor wal/ambassador wal/administrator)
...
(cl-defmacro wal/editors (key fun mfun &rest args)
...
All ARGS are passed to both definers."
  ...
  `(progn
    (wal/editor ,@args ,key ,fun)
    (wal/editor-sink ,@args ,key ,mfun)))
...
(defun wal/major? ()
  ...
  (let ((key (propertize (wal/key-combo-for-leader 'wal/major) 'face 'success))
  ...
  `(defun ,(harpoon--function-name name t) ()
     ...
     ,@(delete
        ...
        `(,(when messages `(wal/message-in-a-bottle ',messages))
          ...
          ,(when major
             `(local-set-key
               (kbd (wal/key-combo-for-leader 'wal/major))
...
(use-package text-mode
  ...
  :init
  (harpoon text-mode
    :functions (wal/flyspell-commit-messages wal/flycheck-on-save))
...
(use-package hideshow
  ...
  :general
  (wal/editor :keymaps 'hs-minor-mode-map
    "h" 'hs-toggle-hiding))
#+END_SRC
...
(use-package dictionary
  ...
  :general
  (wal/administrator "d" 'dictionary-lookup-definition))
#+END_SRC
...
(use-package multiple-cursors
  ...
  :general
  (wal/editors "." 'mc/mark-all-like-this  'mc/edit-ends-of-lines))
#+END_SRC
...
(use-package avy
  ...
  :general
  ;; Lines.
  (wal/editors "c" 'avy-copy-line 'avy-copy-region)
  (wal/editors "x" 'avy-kill-whole-line 'avy-kill-region)
  (wal/editors "m" 'avy-move-line 'avy-move-region)
  (wal/editors "w"
...
The command will be bound using `wal/ambassador' using its initial
    ...
    (wal/transient-define-major org-mode ()
      ...
      [["Act"
       ...
       ["Notes"
        ...
        ("f a" "add footnote" org-footnote-new
         :inapt-if org-in-src-block-p)
        ("f n" "normalize footnotes" org-footnote-normalize)]]))
...
(use-package org-roam
  ...
  (general-define-key
   (wal/key-combo-for-leader 'wal/roamer)
   'roamer)
...
(use-package org-tree-slide
  ...
  :init
  (transient-append-suffix 'org-mode-major '(0 2 -1)
...
(declare-function wal/ambassador "wal-key-bindings.el")
(declare-function wdb/nearby "wal-func.el")
...
(use-package flycheck
  ...
  :init
  (that-key "flycheck" :leader (wal/ambassador :key "f"))
  ...
  (flycheck-keymap-prefix (wal/key-combo-for-leader 'wal/ambassador :key "f" :translate t)))
#+END_SRC
...
(use-package lsp-mode
  ...
  ;; Command map.
  (setq lsp-keymap-prefix (wal/key-combo-for-leader 'wal/ambassador :key "l" :in-sink t))
  (lsp-enable-which-key-integration t)
  (wal/ambassador-sink "l" '(:keymap lsp-command-map :wk "LSP"))
  ...
  :general
  (wal/ambassador :keymaps 'lsp-mode-map "l" '(lsp-mode-dispatch :wk "lsp"))
...
(use-package dap-mode
  ...
  :general
  (wal/ambassador :keymaps 'lsp-mode-map "b" '(dap-hydra :wk "dap"))
...
(use-package verb
  ...
  :general
  (wal/ambassador :keymaps '(verb-mode-map verb-response-body-mode-map)
