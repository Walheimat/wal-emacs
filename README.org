...
Some packages are opt-in and belong to one or several so-called
expansion packs.

These are mostly language-related but will be extended with additional
packages that are not always needed but could be. Any section may
define a pack.
...
(cl-defmacro wal/define-expansion-pack (name &key packages)
  "Define an expansion pack of PACKAGES under NAME."
  `(add-to-list 'wal/expansion-packs '(,name . ,packages)))
...
(defun wal/install-expansion-pack (pack)
  ...
  (let* ((sym (wal/maybe-intern pack))
    ...
    (if item
        (wal/install-packages (cdr item) :on-done (format "Installed expansion pack '%s'." pack))
...
(use-package ligature
  ...
  (ligature-set-ligatures 'elixir-mode '("|>" "->" "<-" "=>"
                                        "::" "<>" "#{" "\\\\"
                                         "===" "!==" ".."))
  (ligature-set-ligatures 'php-mode '("::" "=>" "->" "??" "__"))
...
(use-package org-roam
  ...
  (wal/sergeant
    ...
    "b" '(org-roam-buffer-display-dedicated :wk "buffer")
...
(declare-function wal/biased-random "wal-func.el")
(declare-function wal/define-expansion-pack "wal-func.el")
(declare-function wal/disable-tabs "wal-settings.el")
(declare-function wal/display-buffer-in-side-window "wal-func.el")
(declare-function wal/install-packages "wal-func.el")
...
(wal/define-expansion-pack javacsript
  :packages
  (js2-mode
   rjsx-mode
   typescript-mode
...
(use-package php-mode
  ...
  (wal/display-buffer-in-side-window "\\*Php SOE" :height 15)
