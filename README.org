...
(declare-function wal/lieutenant-sink "wal-key-bindings.el")
...
(text size, windows, scrolling) and one-off actions (like capturing a
note).
...
(defvar wal/popper-hydra-visible nil
  "Non-nil means `wal/popper' is visible.")

(defun wal/popper-handle-window-change ()
  "Activate `wal/popper' when selecting a pop-up window.

If the minibuffer is active, do nothing."
  (unless wal/in-minibuffer
    (with-current-buffer (window-buffer)
      (when (and (fboundp 'wal/popper/body)
                 (popper-popup-p (current-buffer)))
        (unless wal/popper-hydra-visible
          (wal/popper/body))))))
...
(defun wal/configure-popper ()
  ...
  (defhydra wal/popper (:hint nil
                        :timeout 1
                        :foreign-keys warn
                        :pre (setq wal/popper-hydra-visible t)
                        :post (setq wal/popper-hydra-visible nil))
    "
...
"
    ...
    ("q" nil "quit"))
  (add-hook 'buffer-list-update-hook #'wal/popper-handle-window-change))
