...
respective section's package[fn:6]. Each package/section defines a group for
...
(declare-function wal/increase-gc-cons-threshold "wal-func.el")
...
#+BEGIN_SRC emacs-lisp
(defconst wal/modern-emacs (> emacs-major-version 28)
...
It gives access to minor-mode[fn:12] commands and =transient= maps that are
...
#+BEGIN_HTML
<a href='https://coveralls.io/github/Walheimat/emacs-config?branch=trunk'><img src='https://coveralls.io/repos/github/Walheimat/emacs-config/badge.svg?branch=trunk' alt='Coverage Status' /></a>
#+END_HTML
...
(defun walled-mode--disable ()
  ...
  (let* ((window (selected-window))
         (buffer (window-dedicated-p window)))
...
Will hack local variables unless NO-HACK is non-nil (which will
be the case when called interactively)."
  (interactive "p")
...
several so-called expansion packs[fn:13].
...
#+BEGIN_SRC emacs-lisp
(cl-defmacro wal/hook (name docs &body body &key messages lsp lsp-ignores tabs &allow-other-keys)
...
The rest of the BODY will be spliced into the lambda."
  ...
  (let ((target (intern (concat (symbol-name name) "-hook")))
        (safe-body (cl-loop for (key val) on body by 'cddr
                            unless (memq key '(:messages :lsp :lsp-ignores :tabs))
                            collect key
                            and collect val))
...
** Tests
:PROPERTIES:
:header-args:emacs-lisp: :tangle (expand-file-name "test/wal-test.el" wal/emacs-config-package-path)
:VISIBILITY: folded
:END:
...
(ert-deftest test-wal/split-window-the-other-way ()
  (with-temp-buffer
    ...
    (should-not (windows-sharing-edge (selected-window) 'below))
...
#+BEGIN_SRC emacs-lisp
(ert-deftest test-wal/find-file-delete-other-windows ()
  (let ((test-file "/tmp/some-file")
        (some-buffer (get-buffer-create "*testing*")))
    (make-empty-file test-file)
    (switch-to-buffer some-buffer)
    (split-window-horizontally)
    (wal/find-file-delete-other-windows test-file)
    (should (string-equal (buffer-name) "some-file"))
...
(ert-deftest test-wal/find-fish-config--errors-if-not-found ()
  ...
  (let ((wal/fish-config-locations '()))
    (should-error (wal/find-fish-config))))
#+END_SRC
...
(ert-deftest test-wal/reset-indent-defaults ()
  (wal/reset-indent-defaults)
  (should (eq indent-tabs-mode t)))
#+END_SRC
...
(ert-deftest test-wal/duck-duck-go-region--fails-if-no-region ()
  (should-error (wal/duck-duck-go-region)))
#+END_SRC
...
(ert-deftest test-wal/install-packages ()
  (with-mock-all ((package-install . (lambda (package) package))
    ...
    (should (eq 2 (wal/install-packages '(one two))))
...
(ert-deftest test-wal/install-expansion-pack ()
  (let ((messages '()))
    (with-mock-all ((package-installed-p . (lambda (package) nil))
      ...
      (let ((wal/expansion-packs wal/test-packs))
        (wal/install-expansion-pack "one")
        (should (string-equal (car messages) "Installed expansion pack 'one'"))
        (wal/install-expansion-pack "two")
        (should (string-equal (car messages) "Installed all extras."))))))
...
This is a combination of configurations for built-in packages[fn:14]
...
Protesilaos' super configurable[fn:15] themes.
...
You need to install the icons yourself[fn:16].
  ...
  (transient-define-prefix wal/persp ()
    ...
    [["Buffers"
     ...
     ["Perspectives"
      ...
      ("m" "merge" persp-merge)]
     ["Other"
      ("x" "scratch" persp-switch-to-scratch-buffer)]])
...
(defun wal/instead-only-prompt-if-arg-is-present (arg default-cmd prompt)
...
(use-package projectile
  ...
  ;; Don't prompt for the command unless we're using `C-u'.
  (advice-add
   'projectile-maybe-read-command :override
   #'wal/instead-only-prompt-if-arg-is-present)
...
=vterm= is better[fn:18] than included shells.
    ...
    [["Inferior shell"
     ...
     ["Commands"
      ...
      ("b" "black" python-black-buffer
       :inapt-if-not (lambda () (fboundp 'python-black-buffer)))]
     ["Test"
     ...
     ["Virtual environment"
      ("v" "activate pyvenv" pyvenv-activate
       :inapt-if-not (lambda () (featurep 'pyvenv)))]
     ["Poetry"
      ("p" "poetry" poetry
       :inapt-if-not (lambda () (featurep 'poetry)))]])
...
Takes care[fn:20] of all your virtual environment needs.
...
(wal/define-expansion-pack lisp
  ...
  :packages (lispy highlight-quoted slime))
...
#+begin_src emacs-lisp
(defconst wal/jdtls-fs (concat
                        "https://download.eclipse.org/"
                        "jdtls/milestones/%1$s/"
                        "jdt-language-server-%1$s-%2$s.tar.gz")
...
(defun wal/ascii-whale-setup ()
  ...
  (when (featurep 'posframe)
    ...
    (with-current-buffer (get-buffer-create wal/ascii-whale-buffer)
      (face-remap-add-relative 'default :height 240))
...
(defun wal/show-config-diff-range ()
  ...
  (let ((version (shell-command-to-string "git describe --abbrev=0")))
    ...
    (magit-diff-range (string-trim version))))
...
(defun wal/check-coverage ()
  ...
  (let* ((dir wal/emacs-config-package-path)
         (command (format "cd %s && cask exec ert-runner && cat coverage.txt" dir))
         (result (shell-command-to-string command))
         (failure (string-match "FAILED" result)))

    (if failure
        (message (string-trim (substring result failure)))
      (message "%s" (car (last (seq-filter
...
(package "wal" "1.7.11" "Walheimat's configuration")
...
[fn:6] I'll eventually turn everything into a real package.
...
[fn:12] Provided the modes are active.

[fn:13] Alternate spelling is "expansion pak".

[fn:14] The following packages (and their related or sub-packages) are
...
[fn:15] Unfortunately these themes require to the customizations to be
set before loading, which means that when using these themes you have
to load them twice.

[fn:16] The =all-the-icons= icons need to be downloaded manually by
...
[fn:17] You might have to call =emojify-download-emoji= to download a
set that supports your emojis.

[fn:18] If you're on an older distro your =libvterm= package might be
too old.

[fn:19] None of the language packages are ensured, meaning you will
need to install them yourself.

[fn:20] Currently requires [[https://daseldocs.tomwright.me/installation#manual ][dasel]].

[fn:21] Binding reference [[http://oremacs.com/lispy/][here]].

[fn:22] All languages listed [[https://emacs-lsp.github.io/lsp-mode/page/languages/][here]].
