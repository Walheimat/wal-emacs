...
You can install either a =systemd= service using [[file:templates/emacs.service][this template]][fn:5] or
just execute =emacs --daemon= in a terminal (or your login shell's
configuration).
...
that is defined in the [[file:setup/wal-prelude.el][prelude]] package.
