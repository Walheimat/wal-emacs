...
(use-package avy
  ...
  :general
  (wal/colonel
    "l" '(avy-goto-word-1 :wk "go to word"))
  (wal/colonel-sink
    "l" '(avy-goto-word-0 :wk "go to any word"))
  (wal/lieutenant
    ;; Lines.
...
(defun wal/configure-lsp-server (venv)
  "Configure server for `lsp-mode' using poetry."
  (let ((pylsp (file-exists-p (expand-file-name "bin/pylsp" venv)))
    ...
    (when pyls
      ...
      (setq-local lsp-pyls-plugins-jedi-environment jedi-env))))
