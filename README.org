...
Almost all hyper bindings are quick-access actions:
...
+ =H-l= jumps to line (or char using =L=) with =avy=
...
It's also used it for certain dispatches that I use
often enough that one key-press less is important:
...
The six =general= mappings perform all non-standard actions.
...
(defvar wal/sergeant-key "H-["
  "Wal's senary (or sergeant) leader key.")
...
(use-package general
  ...
  (general-create-definer wal/sergeant
    :prefix wal/sergeant-key))
#+END_SRC
...
Leader key =sergeant= is for paraphernalia.
Assorted actions that I sometimes need.
...
(wal/general "c" '(wal/open-config :wk "config")
             "g" '(wal/tangle-config :wk "tangle"))
...
(use-package org-roam
  ...
  :general
  (wal/lieutenant "z" '(:ignore t :wk "org-roam")
...
(use-package perspective
  ...
  (:map persp-mode-map
   ...
   ("H-k" . kmacro-keymap))) ; Make it available again.
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
(defun wal/magit-display-buffer (buffer)
...
(use-package magit
  :custom
  (magit-display-buffer-function 'wal/magit-display-buffer)
  :bind
  (("C-c g" . magit-status)
...
(use-package pyvenv
  :general
  (wal/sergeant :keymaps '(dired-mode-map python-modemap)
...
(use-package multiple-cursors
  ...
  :general
  (wal/lieutenant "." '(:ignore t :wk "multiple cursors")
    ".a" '(mc/mark-all-like-this :wk "all like this")
    ".r" '(:ignore t :wk "region")
    ".rb" '(mc/edit-beginnings-of-lines :wk "beginnings")
...
(use-package crux
  :general
  (wal/general "i" '(crux-find-user-init-file :wk "init file"))
  (wal/sergeant "r" '(crux-rename-file-and-buffer :wk "rename file and buffer"))
  (wal/sergeant "s" '(crux-sudo-edit :wk "sudo edit"))
  :bind
...
(use-package avy
  ...
  (wal/lieutenant
    ...
    "lr" '(avy-copy-region :wk "copy region" ))
  :bind
  (("H-l" . avy-goto-line)
   ("H-L" . avy-goto-char)))
#+END_SRC
...
(use-package consult
  ...
  (wal/sergeant "l" '(consult-focus-lines :wk "focus lines")))

(use-package consult-flycheck
  :after (conult flycheck)
  :config
...
(use-package ag
  ...
  (ag-ignore-list '(".git"
                    ...
                    "*.svg"))
  :general
  (wal/lieutenant "s" '(:ignore t :wk "ag")
    "sd" '(ag :wk "directory")
    "sf" '(ag-files :wk "files")
    "sp" '(ag-project :wk "project"))
  (wal/lieutenant :keymaps 'dired-mode-map
    "sd" '(ag-dired :wk "dired"))
  :bind ("H-s" . ag))
#+END_SRC
...
(use-package counsel
  ...
  (wal/sergeant "g" '(counsel-search :wk "duck-duck-go"))
  :bind
  (:map ivy-mode-map
...
(use-package eww
  :ensure nil
  :general
  (wal/lieutenant "w" '(:ignore t :wk "eww")
                  "wb" '(eww-list-bookmarks :wk "bookmarks")
                  "ww" '(eww :wk "browse")))
#+END_SRC
